...
function enforceOwnedModel(model: string, userId: string): ScopedModel {
  ...
  return {
    ...
    count:     (args = {}) => m.count({ ...args,     where: withUserWhere(args.where, userId) }),
...
export function scopedPrisma(userId: string) {
  return {
    ...
    fileFolder:  enforceOwnedModel("fileFolder",  userId),
