...
const DEFAULT_COLORS = { A:"#c084fc", BG:"#1e0a3e", HD:"#3c2068", S2:"#280d50", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Lavender() {
  const [data, setData] = usePersistentCLState<CLData>('cl_lavender_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_lavender_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_lavender_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_lavender_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#1e0a3e", padding: "24px 16px", fontFamily: fnt }}>
      ...
      <div className="cllav-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
