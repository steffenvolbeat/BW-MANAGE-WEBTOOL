"use client";
// ─── Anschreiben Template: Solar (Orange/Braun) ───────────────────────────────
...
const A = "#f97316"; const DARK = "#431407";
...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(255,255,255,0.1)" : "rgba(249,115,22,0.05)", border: `1px dashed ${inv ? "rgba(255,255,255,0.4)" : `${A}55`}`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Solar() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1c0a00", padding: "24px 16px" }}>
      <style>{`${curFont.gf ? `@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');` : ""} .clsol-doc,.clsol-doc * { font-family: ${fnt} !important; } @media print { @page{size:A4 portrait;margin:0} body *{visibility:hidden!important} .clsol-doc,.clsol-doc *{visibility:visible!important} .clsol-doc{position:absolute!important;top:0!important;left:0!important;width:210mm!important;box-shadow:none!important;margin:0!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important} .clsol-zoom{zoom:1!important;width:100%!important} .clsol-ctrl{display:none!important} }`}</style>
      <div className="clsol-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {showDesign && <div style={{ width: "100%", background: DARK, border: `1px solid ${A}44`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: A, marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#7c2d12"}`, background: fontKey === f.key ? "rgba(249,115,22,0.12)" : "transparent", color: fontKey === f.key ? A : "#9ca3af", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: A, marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Größe</div><div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#7c2d12"}`, background: sizeKey === s.key ? "rgba(249,115,22,0.12)" : "transparent", color: sizeKey === s.key ? A : "#9ca3af", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div></div>
        </div>}
      </div>
      <div className="clsol-doc" style={{ width: 850, margin: "0 auto", backgroundColor: "white", boxShadow: `0 8px 50px rgba(249,115,22,0.2)`, overflow: "hidden" }}>
        <div className="clsol-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          <div style={{ background: `linear-gradient(135deg,${DARK} 0%,${A} 100%)`, padding: "28px 48px 22px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              <div><E value={data.personal.name} onChange={v => setP({ name: v })} editing={editing} inv style={{ fontSize: 28, fontWeight: 900, color: "white", display: "block", marginBottom: 3 }} /><E value={data.personal.subtitle} onChange={v => setP({ subtitle: v })} editing={editing} inv style={{ fontSize: 11, color: "rgba(255,255,255,0.8)", display: "block" }} /></div>
          ...
          <div style={{ height: 3, background: `linear-gradient(90deg,${A}cc,${DARK}cc)` }} />
          <div style={{ padding: "32px 48px 48px", color: "#374151" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
              <div style={{ borderLeft: `3px solid ${A}`, paddingLeft: 10 }}>{[data.recipient.company, data.recipient.street, data.recipient.cityZip].map((v, i) => <E key={i} value={v} onChange={nv => setR([{ company: nv }, { street: nv }, { cityZip: nv }][i])} editing={editing} style={{ fontSize: 11, color: "#374151", display: "block", lineHeight: 1.6 }} />)}</div>
              <E value={data.date} onChange={v => setData(d => ({ ...d, date: v }))} editing={editing} style={{ fontSize: 11, color: "#9ca3af", fontStyle: "italic" }} />
            </div>
            <div style={{ marginBottom: 20, paddingBottom: 12, borderBottom: `2px solid ${A}66` }}>
