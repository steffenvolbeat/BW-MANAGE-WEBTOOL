"use client";
// ─── Anschreiben Template: Obsidian (Grau-Mono) ────────────────────────────
...
const DEFAULT_COLORS = { A:"#6b7280", BG:"#111827", HD:"#293348", S2:"#1f2937", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Obsidian() {
  const [data, setData] = usePersistentCLState<CLData>('cl_obsidian_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_obsidian_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_obsidian_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_obsidian_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#111", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
        ...
        .clobs-doc, .clobs-doc * { font-family: ${fnt} !important; }
        @media print {
      ...
      <div className="clobs-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
