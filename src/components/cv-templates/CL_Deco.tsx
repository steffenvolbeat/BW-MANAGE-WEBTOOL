"use client";
// ─── Anschreiben Template: Art Deco (Gold auf Schwarz) ───────────────────────
...
const DEFAULT_COLORS = { A:"#d4af37", BG:"#0d0d0d", HD:"#0d0d0d", S2:"#1a1a1a", CT:"#f5e6c8", CB:"#d4c5a9", CM:"#7a6a4a" };
...
export default function CL_Deco() {
  const [data,setData]=usePersistentCLState<CLData>("cl_deco_data",JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState("cl_deco_font","playfair");
  const [sizeKey,setSizeKey]=usePersistentCLState("cl_deco_size","md");
  const [showDesign,setShowDesign]=useState(false);
  const [clrs,setClrs]=usePersistentCLState("cl_deco_colors",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#050505",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
        ...
        .cldco-doc,.cldco-doc *{font-family:${fnt}!important;}
        @media print{
          @page{size:A4 portrait;margin:0;}
          *,*::before,*::after{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
          body *{visibility:hidden!important;}
          .cldco-doc,.cldco-doc *{visibility:visible!important;}
          .cldco-doc{position:absolute!important;top:0!important;left:0!important;width:850px!important;min-height:1202px!important;overflow:visible!important;zoom:0.934!important;box-shadow:none!important;margin:0!important;}
          .cldco-zoom{zoom:1!important;width:100%!important;}
          .cldco-ctrl{display:none!important;}
        }
      `}</style>
      <div className="cldco-ctrl" style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,fontWeight:600,cursor:"pointer",border:`1px solid ${A}`,borderRadius:4,backgroundColor:hex2rgba(A,0.12),color:A,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`1px solid ${showDesign?A:"#555"}`,borderRadius:4,backgroundColor:"transparent",color:showDesign?A:"#999"}}>🎨 Design</button>
        <button onClick={doReset} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:"1px solid #555",borderRadius:4,backgroundColor:"transparent",color:"#999",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(<div style={{width:"100%",background:"#111",border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:4,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
      ...
      <div className="cldco-doc" style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:`0 4px 30px rgba(212,175,55,0.2)`,fontFamily:fnt,overflow:"hidden",border:`1px solid ${hex2rgba(A,0.15)}`}}>
        <div className="cldco-zoom" style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{background:`linear-gradient(90deg,transparent,${A},transparent)`,height:2}}/>
          <div style={{backgroundColor:HD,padding:"44px 60px 32px",textAlign:"center"}}>
            <div style={{fontSize:11,letterSpacing:"0.4em",color:A,textTransform:"uppercase",marginBottom:12}}>◆ Bewerbung ◆</div>
            <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:40,fontWeight:700,color:CT,display:"block",letterSpacing:"0.04em"}}/>
            <div style={{background:`linear-gradient(90deg,transparent,${A},transparent)`,height:1,margin:"14px 0 10px"}}/>
            <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:12,color:A,display:"block",fontWeight:400,letterSpacing:"0.2em",textTransform:"uppercase"}}/>
            <div style={{display:"flex",gap:24,flexWrap:"wrap",marginTop:14,justifyContent:"center"}}>{(["email","phone","location","website","linkedin","github"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CLData["personal"]>)} style={{fontSize:10,color:CM,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.2)}`,padding:"2px 8px",outline:"none",fontFamily:"inherit",borderRadius:2}}/>:<span key={k} style={{fontSize:10,color:CM}}>{data.personal[k]}</span>:null)}</div>
          </div>
          <div style={{background:`linear-gradient(90deg,transparent,${A},transparent)`,height:1}}/>
          <div style={{padding:"36px 60px 52px",color:CB}}>
            ...
            <div style={{marginBottom:24,textAlign:"center",paddingBottom:16,borderBottom:`1px solid ${hex2rgba(A,0.3)}`}}><E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:16,fontWeight:700,color:A,letterSpacing:"0.08em"}}/></div>
            ...
            <div style={{marginTop:40}}><E value={data.closing} onChange={v=>setData(d=>({...d,closing:v}))} editing={editing} style={{fontSize:12,color:CB,display:"block",marginBottom:44}}/><div style={{background:`linear-gradient(90deg,${A},${hex2rgba(A,0.3)})`,height:1,width:150,marginBottom:12}}/><E value={data.signatureName} onChange={v=>setData(d=>({...d,signatureName:v}))} editing={editing} style={{fontSize:18,fontWeight:700,color:CT}}/></div>
          </div>
          <div style={{background:`linear-gradient(90deg,transparent,${A},transparent)`,height:2}}/>
        </div>
      </div>
