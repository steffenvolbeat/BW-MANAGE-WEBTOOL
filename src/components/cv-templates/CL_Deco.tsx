      ...
      <div className="cldco-doc" style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:`0 4px 30px rgba(212,175,55,0.2)`,fontFamily:fnt,overflow:"hidden",border:`1px solid ${hex2rgba(A,0.15)}`}}>
        <div className="cldco-zoom" style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          <div style={{backgroundColor:HD,padding:"44px 60px 32px",textAlign:"center"}}>
            ...
            <div style={{display:"flex",gap:24,flexWrap:"wrap",marginTop:14,justifyContent:"center"}}>{(["email","phone","location","website"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CLData["personal"]>)} style={{fontSize:10,color:CM,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.2)}`,padding:"2px 8px",outline:"none",fontFamily:"inherit",borderRadius:2}}/>:<span key={k} style={{fontSize:10,color:CM}}>{data.personal[k]}</span>:null)}</div>
          </div>
