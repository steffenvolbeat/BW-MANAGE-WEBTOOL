...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 3 }: {
  ...
  const s: React.CSSProperties = { ...style, background: "rgba(251,191,36,0.08)", border: "1px dashed rgba(251,191,36,0.4)", borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
        ...
        <li key={i} style={{ display: "flex", alignItems: "flex-start", gap: 6, marginBottom: 3 }}>
          ...
          {editing ? (
            <div style={{ flex: 1, display: "flex", gap: 4 }}>
              <input value={b} onChange={e => { const n = [...bullets]; n[i] = e.target.value; onChange(n); }} style={{ flex: 1, fontSize: 11, color: CB, background: S3, border: "1px dashed rgba(251,191,36,0.3)", borderRadius: 3, padding: "1px 4px", outline: "none", fontFamily: "inherit" }} />
...
function TagList({ tags, onChange, editing }: { tags: string[]; onChange: (t: string[]) => void; editing: boolean }) {
  ...
  return (
    <div style={{ display: "flex", flexWrap: "wrap", gap: 5 }}>
      ...
      {editing && <input value={nv} onChange={e => setNv(e.target.value)} onKeyDown={e => { if (e.key === "Enter" && nv.trim()) { onChange([...tags, nv.trim()]); setNv(""); } }} style={{ width: 70, fontSize: 10, border: "1px dashed rgba(251,191,36,0.4)", borderRadius: 3, padding: "2px 5px", background: "rgba(251,191,36,0.08)", outline: "none", color: CT, fontFamily: "inherit" }} placeholder="+ Neu" />}
    </div>
  );
}
...
export default function CV_Gold() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f0a00", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setShowDesign(v => !v)} style={{ padding: "7px 16px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: `1px solid ${showDesign ? A : "#333"}`, backgroundColor: showDesign ? "rgba(251,191,36,0.1)" : "transparent", color: showDesign ? A : "#888" }}>
          🎨 Design
        </button>
          ...
          <div style={{ width: "100%", background: S2, border: "1px solid #333", borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
            <div>
              ...
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>
                {FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#333"}`, background: fontKey === f.key ? "rgba(251,191,36,0.12)" : "transparent", color: fontKey === f.key ? A : "#888", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}
              </div>
            </div>
            <div>
              ...
              <div style={{ display: "flex", gap: 5 }}>
                {FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#333"}`, background: sizeKey === s.key ? "rgba(251,191,36,0.12)" : "transparent", color: sizeKey === s.key ? A : "#888", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}
              </div>
            </div>
            <div>
      ...
      <div className={`${PFX}-doc`} style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: "0 0 60px rgba(251,191,36,0.12)", overflow: "hidden", fontFamily: fnt }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginTop: 10 }}>
                {(["email", "phone", "location", "github", "linkedin", "website"] as const).map(k => editing
                  ? <input key={k} value={data.personal[k] ?? ""} onChange={e => setP({ [k]: e.target.value } as Partial<CVData["personal"]>)} style={{ fontSize: 10, color: CB, background: S3, border: "1px dashed rgba(251,191,36,0.3)", borderRadius: 4, padding: "2px 8px", outline: "none", fontFamily: fnt }} />
