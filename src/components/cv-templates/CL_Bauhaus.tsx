      ...
      <div className="clbhs-doc" style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(0,0,0,0.15)",fontFamily:fnt,overflow:"hidden"}}>
        <div className="clbhs-zoom" style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          <div style={{backgroundColor:HD,padding:"32px 52px 24px"}}>
            ...
            <div style={{display:"flex",gap:24,flexWrap:"wrap",marginTop:16}}>{(["email","phone","location","website"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CLData["personal"]>)} style={{fontSize:10,color:"rgba(245,245,245,0.8)",background:"rgba(255,255,255,0.08)",border:"1px solid rgba(255,255,255,0.2)",padding:"2px 8px",outline:"none",fontFamily:"inherit"}}/>:<span key={k} style={{fontSize:10,color:"rgba(245,245,245,0.7)"}}>{data.personal[k]}</span>:null)}</div>
          </div>
          {/* Body */}
