"use client";
// ─── Anschreiben Template: StainedGlass ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#f59e0b", BG:"#1a0a00", HD:"#240e00", S2:"#301500", CT:"#fff8e8", CB:"#f0d8a0", CM:"#c09040" };
...
export default function CL_StainedGlass(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clstgls_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clstgls_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clstgls_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clstgls_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#100600",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 40px rgba(245,158,11,0.3)",overflow:"visible",position:"relative"}}>
