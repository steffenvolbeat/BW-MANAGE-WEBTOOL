"use client";
// ─── Anschreiben Template: Dashboard ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#6366f1", BG:"#0f172a", HD:"#1e293b", S2:"#293548", CT:"#f1f5f9", CB:"#cbd5e1", CM:"#64748b" };
...
export default function CL_Dashboard(){
  const [data,setData]=usePersistentCLState<CLData>('cl_cldash_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_cldash_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_cldash_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_cldash_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#080f1e",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 40px rgba(99,102,241,0.25)",overflow:"visible",position:"relative"}}>
