...
const DEFAULT_COLORS = { A:"#00c8ff", BG:"#0a1628", S2:"#0f1f3d", S3:"#162a50", SBG:"#112236", CT:"#e0f4ff", CB:"#b0d8f0", CM:"#6899bb" };
...
function BulletList({bullets,onChange,editing}:{bullets:string[];onChange:(b:string[])=>void;editing:boolean}){
  ...
  return(<ul style={{listStyle:"none",margin:0,padding:0}}>{bullets.map((b,i)=>(
    <li key={i} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
      <span style={{width:0,height:0,borderLeft:`5px solid ${A}`,borderTop:"4px solid transparent",borderBottom:"4px solid transparent",flexShrink:0,marginTop:4}}/>
...
function TagList({tags,onChange,editing}:{tags:string[];onChange:(t:string[])=>void;editing:boolean}){
  const {A,CT,BG}=useContext(ColCtx);const [nv,setNv]=useState("");
  return(<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map((t,i)=>(<div key={i} style={{background:hex2rgba(A,0.15),border:`1px solid ${A}55`,color:CT,padding:"2px 8px",fontSize:10,display:"flex",alignItems:"center",gap:3,clipPath:"polygon(6px 0%,100% 0%,calc(100% - 6px) 100%,0% 100%)"}}>{t}{editing&&<button type="button" onClick={()=>onChange(tags.filter((_,j)=>j!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#f87171",padding:0}}><XMarkIcon style={{width:9,height:9}}/></button>}</div>))}
  {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px dashed ${A}`,padding:"2px 5px",background:hex2rgba(A,0.05),outline:"none",color:CT,fontFamily:"inherit"}} placeholder="+ Neu"/>}</div>);
}
function SecH({title}:{title:string}){
  ...
  return(<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
    <div style={{width:16,height:16,position:"relative",flexShrink:0}}>
      <div style={{position:"absolute",width:12,height:12,background:A,transform:"skewX(-15deg) skewY(-15deg)",top:2,left:2}}/>
      <div style={{position:"absolute",width:12,height:12,border:`1px solid ${A}66`,transform:"skewX(-15deg) skewY(-15deg)",top:0,left:0}}/>
    </div>
    <span style={{fontSize:11,fontWeight:800,letterSpacing:"0.14em",textTransform:"uppercase" as const,color:CT}}>{title}</span>
    <div style={{flex:1,height:1,background:`linear-gradient(90deg,${A}55,transparent)`}}/>
  </div>);
}

export default function CV_Isometric(){
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("isometric",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#061020",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div ref={docRef} className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,overflow:"visible",fontFamily:fnt,position:"relative",boxShadow:"0 0 60px rgba(0,200,255,0.2)"}}>
        ...
        <svg style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",opacity:0.06,pointerEvents:"none"}} xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="isoGrid" width="40" height="40" patternUnits="userSpaceOnUse" patternTransform="rotate(30)"><line x1="0" y1="0" x2="0" y2="40" stroke={A} strokeWidth="0.5"/><line x1="0" y1="20" x2="40" y2="20" stroke={A} strokeWidth="0.5"/></pattern></defs>
        ...
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale,position:"relative"}}>
          {/* Header */}
          <div style={{background:`linear-gradient(135deg,${S2},${S3})`,padding:"32px 40px 24px",borderBottom:`3px solid ${A}`,position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",top:-20,right:-20,width:120,height:120,border:`2px solid ${A}22`,transform:"rotate(45deg) skew(15deg,15deg)"}}/>
            <div style={{position:"absolute",top:-10,right:-10,width:80,height:80,border:`1px solid ${A}44`,transform:"rotate(45deg) skew(15deg,15deg)"}}/>
            <div style={{display:"flex",gap:28,alignItems:"center"}}>
              <div style={{position:"relative",width:curShape.w,height:curShape.h,flexShrink:0,cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
                <div style={{width:"100%",height:"100%",borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",backgroundColor:S3,border:`2px solid ${A}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 20px ${hex2rgba(A,0.4)}`}}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:30,fontWeight:900,color:CT,marginBottom:4,fontFamily:"inherit",background:hex2rgba(A,0.08),border:`1px dashed ${A}`,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:30,fontWeight:900,color:CT,marginBottom:4,letterSpacing:"0.02em",textShadow:`0 0 20px ${hex2rgba(A,0.5)}`}}>{data.personal.name}</div>}
                {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:13,color:A,marginBottom:8,fontFamily:"inherit",background:hex2rgba(A,0.08),border:`1px dashed ${A}`,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:13,fontWeight:700,color:A,marginBottom:8,letterSpacing:"0.1em",textTransform:"uppercase" as const}}>{data.personal.subtitle}</div>}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:10}}>
                  {(["email","phone","location","github","linkedin","website"] as const).map(k=>editing
                    ...
                    :data.personal[k]?<span key={k} style={{fontSize:10,color:A,background:hex2rgba(A,0.1),padding:"2px 8px",border:`1px solid ${A}33`,clipPath:"polygon(4px 0%,100% 0%,calc(100% - 4px) 100%,0% 100%)"}}>{data.personal[k]}</span>:null)}
                </div>
              </div>
            </div>
          </div>
          <div style={{display:"flex"}}>
            ...
            <div style={{width:220,background:S2,padding:"22px 18px",flexShrink:0,borderRight:`1px solid ${A}22`}}>
              ...
              <div style={{marginBottom:20}}><SecH title="Sprachen"/>
                {data.languages.map((l,i)=>(<div key={i} style={{marginBottom:8}}>
                  ...
                  <div style={{height:3,background:`${A}22`}}><div style={{height:"100%",width:l.level==="Muttersprache"?"100%":l.level==="Verhandlungssicher"?"85%":l.level==="Fließend"?"70%":l.level==="Grundkenntnisse"?"40%":"60%",background:`linear-gradient(90deg,${A},${hex2rgba(A,0.5)})`,boxShadow:`0 0 6px ${A}`}}/></div>
                </div>))}
              </div>
              <div style={{marginBottom:20}}><SecH title="Ausbildung"/>
                {data.education.map(e=>(<div key={e.id} style={{marginBottom:12,paddingLeft:8,borderLeft:`2px solid ${A}55`}}>
            ...
            <div style={{flex:1,padding:"22px 28px"}}>
              <div style={{marginBottom:20}}><SecH title="Berufserfahrung"/>
                {data.experience.map(ex=>(<div key={ex.id} style={{marginBottom:16,paddingLeft:14,borderLeft:`2px solid ${A}44`,position:"relative"}}>
                  <div style={{position:"absolute",left:-6,top:4,width:10,height:10,background:A,transform:"rotate(45deg) skew(10deg,10deg)",boxShadow:`0 0 8px ${A}`}}/>
              ...
              <div style={{marginBottom:20}}><SecH title="Projekte"/>
                {data.projects.map(p=>(<div key={p.id} style={{marginBottom:14,paddingLeft:14,borderLeft:`2px solid ${A}33`,position:"relative"}}>
                  <div style={{position:"absolute",left:-6,top:4,width:10,height:10,background:`${A}66`,transform:"rotate(45deg) skew(10deg,10deg)"}}/>
