...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(148,163,184,0.08)" : "rgba(148,163,184,0.06)", border: `1px dashed ${A}55`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Slate() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0f172a", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
        ...
        .clslt-doc, .clslt-doc * { font-family: ${fnt} !important; }
        @media print {
          @page { size: A4 portrait; margin: 0; }
          body * { visibility: hidden !important; }
          .clslt-doc, .clslt-doc * { visibility: visible !important; }
          .clslt-doc { position: absolute !important; top: 0 !important; left: 0 !important; width: 210mm !important; box-shadow: none !important; margin: 0 !important; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; }
          .clslt-zoom { zoom: 1 !important; width: 100% !important; }
          .clslt-ctrl { display: none !important; }
        }
      `}</style>
      <div className="clslt-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {showDesign && (
          <div style={{ width: "100%", background: S2, border: "1px solid #333", borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
            <div><div style={{ fontSize: 10, fontWeight: 700, color: "#555", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div>
              <div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#333"}`, background: fontKey === f.key ? "rgba(148,163,184,0.1)" : "transparent", color: fontKey === f.key ? A : "#888", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div>
            </div>
            <div><div style={{ fontSize: 10, fontWeight: 700, color: "#555", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftgröße</div>
              <div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#333"}`, background: sizeKey === s.key ? "rgba(148,163,184,0.1)" : "transparent", color: sizeKey === s.key ? A : "#888", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div>
            </div>
          </div>
        )}
      </div>

      <div className="clslt-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px rgba(148,163,184,0.15)`, overflow: "hidden" }}>
        <div className="clslt-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          {/* Header */}
