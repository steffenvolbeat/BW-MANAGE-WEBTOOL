...
const DEFAULT_COLORS = { A:"#fb923c", BG:"#1a0600", HD:"#361a12", S2:"#220800", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Ember() {
  const [data, setData] = usePersistentCLState<CLData>('cl_ember_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_ember_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_ember_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_ember_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#1a0600", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
        ...
        @media print {
          ...
          *, *::before, *::after { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
          ...
          .clemb-doc { position: absolute !important; top: 0 !important; left: 0 !important; width: 850px !important; min-height: 1202px !important; overflow: hidden !important; zoom: 0.934 !important; box-shadow: none !important; margin: 0 !important; }
      ...
      <div className="clemb-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
