...
const DEFAULT_COLORS = { A:"#d4af37", BG:"#0d0d0d", S2:"#1a1a1a", S3:"#252525", SBG:"#111111", CT:"#f5e6c8", CB:"#d4c5a9", CM:"#7a6a4a" };
...
function BulletList({bullets,onChange,editing}:{bullets:string[];onChange:(b:string[])=>void;editing:boolean}) {
  ...
  return(<ul style={{listStyle:"none",margin:0,padding:0}}>{bullets.map((b,i)=>(<li key={i} style={{display:"flex",alignItems:"flex-start",gap:8,marginBottom:3}}><span style={{color:A,fontSize:12,marginTop:1,flexShrink:0}}>◆</span>{editing?(<div style={{flex:1,display:"flex",gap:4}}><input value={b} onChange={e=>{const n=[...bullets];n[i]=e.target.value;onChange(n);}} style={{flex:1,fontSize:11,color:CB,background:hex2rgba(A,0.07),border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:2,padding:"1px 4px",outline:"none",fontFamily:"inherit"}}/><button type="button" onClick={()=>onChange(bullets.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:0,display:"flex"}}><XMarkIcon style={{width:11,height:11}}/></button></div>):<span style={{fontSize:11,color:CB,lineHeight:1.7}}>{b}</span>}</li>))}{editing&&<li><button type="button" onClick={()=>onChange([...bullets,""])} style={{display:"flex",alignItems:"center",gap:4,fontSize:11,color:A,background:"none",border:"none",cursor:"pointer",padding:"3px 0",fontFamily:"inherit"}}><PlusIcon style={{width:11,height:11}}/>Hinzufügen</button></li>}</ul>);
}
function TagList({tags,onChange,editing}:{tags:string[];onChange:(t:string[])=>void;editing:boolean}) {
  const {A,CB}=useContext(ColCtx);
  const [nv,setNv]=useState("");
  return(<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map((t,i)=>(<div key={i} style={{border:`1px solid ${A}`,color:A,borderRadius:0,padding:"2px 9px",fontSize:10,display:"flex",alignItems:"center",gap:3,letterSpacing:"0.06em"}}>{t}{editing&&<button type="button" onClick={()=>onChange(tags.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:0,display:"flex"}}><XMarkIcon style={{width:9,height:9}}/></button>}</div>))}{editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px solid ${hex2rgba(A,0.4)}`,borderRadius:0,padding:"2px 8px",background:hex2rgba(A,0.07),outline:"none",color:CB,fontFamily:"inherit"}} placeholder="+ Neu"/>}</div>);
}
function SecH({title}:{title:string}) {
  const {A}=useContext(ColCtx);
  return(<div style={{marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4}}><span style={{color:A,fontSize:10}}>◆</span><span style={{fontSize:10,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase" as const,color:A}}>{title}</span><span style={{color:A,fontSize:10}}>◆</span></div><div style={{height:1,background:`linear-gradient(90deg,transparent,${A},transparent)`}}/></div>);
}

export default function CV_Deco() {
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("deco",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#050505",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>setEditing(e=>!e)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 16px",borderRadius:2,fontSize:13,fontWeight:600,cursor:"pointer",border:`1px solid ${editing?"#16a34a":A}`,backgroundColor:editing?"#16a34a":hex2rgba(A,0.15),color:editing?"white":A}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
        <button onClick={()=>window.print()} style={{padding:"7px 16px",borderRadius:2,fontSize:13,fontWeight:600,cursor:"pointer",border:`1px solid ${A}`,backgroundColor:hex2rgba(A,0.1),color:A,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",borderRadius:2,fontSize:13,cursor:"pointer",border:`1px solid ${showDesign?A:"#444"}`,backgroundColor:"transparent",color:showDesign?A:"#888"}}>🎨 Design</button>
        <button onClick={()=>{resetStorage();setData(JSON.parse(JSON.stringify(DEFAULT_CV_DATA)));setPhotoSrc("");setFontKey("playfair");setSizeKey("md");setPhotoShapeKey("circle");setClrs(DEFAULT_COLORS);setShowDesign(false);}} style={{padding:"7px 16px",borderRadius:2,fontSize:13,cursor:"pointer",border:"1px solid #444",backgroundColor:"transparent",color:"#888",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(<div style={{width:"100%",background:"#111",border:`1px solid ${A}`,borderRadius:4,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
          <div><div style={{fontSize:10,fontWeight:700,color:A,marginBottom:7,letterSpacing:"0.15em",textTransform:"uppercase"}}>Schriftart</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{FONTS.map(f=><button key={f.key} onClick={()=>setFontKey(f.key)} style={{padding:"3px 9px",borderRadius:2,border:`1px solid ${fontKey===f.key?A:"#333"}`,background:fontKey===f.key?hex2rgba(A,0.15):"transparent",color:fontKey===f.key?A:CM,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:f.family}}>{f.label}</button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:A,marginBottom:7,letterSpacing:"0.15em",textTransform:"uppercase"}}>Schriftgröße</div><div style={{display:"flex",gap:5}}>{FONT_SIZES.map(s=><button key={s.key} onClick={()=>setSizeKey(s.key)} style={{padding:"3px 12px",borderRadius:2,border:`1px solid ${sizeKey===s.key?A:"#333"}`,background:sizeKey===s.key?hex2rgba(A,0.15):"transparent",color:sizeKey===s.key?A:CM,fontSize:11,cursor:"pointer"}}>{s.label}</button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:A,marginBottom:7,letterSpacing:"0.15em",textTransform:"uppercase"}}>Foto-Form</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{PHOTO_SHAPES.map(s=><button key={s.key} onClick={()=>setPhotoShapeKey(s.key)} title={s.label} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"5px 7px",borderRadius:2,border:`1px solid ${photoShapeKey===s.key?A:"#333"}`,background:photoShapeKey===s.key?hex2rgba(A,0.1):"transparent",cursor:"pointer"}}><div style={{width:20,height:s.key==="ellipse"?26:20,borderRadius:s.br,clipPath:s.clip??"",backgroundColor:photoShapeKey===s.key?A:S3}}/><span style={{fontSize:8,color:photoShapeKey===s.key?A:CM,whiteSpace:"nowrap"}}>{s.label}</span></button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:A,marginBottom:7,letterSpacing:"0.15em",textTransform:"uppercase"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Gold"},{k:"BG" as const,l:"BG"},{k:"S2" as const,l:"Header"},{k:"S3" as const,l:"Karte"},{k:"SBG" as const,l:"Sidebar"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Meta"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,borderRadius:2,border:`1px solid ${A}`,cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:CM}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",borderRadius:2,fontSize:11,border:`1px solid ${A}`,background:"transparent",color:A,cursor:"pointer",alignSelf:"center"}}>↺ Reset</button></div></div>
        </div>)}
      </div>
      ...
      <div className={`${PFX}-doc`} style={{width:850, minHeight: 1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 0 40px ${hex2rgba(A,0.2)}`,fontFamily:fnt,overflow: "visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          <div style={{height:1,background:hex2rgba(A,0.3),margin:"3px 40px"}}/>
          {/* Header - centered */}
          <div style={{backgroundColor:S2,padding:"36px 56px 28px",textAlign:"center",borderBottom:`1px solid ${hex2rgba(A,0.3)}`}}>
            <div style={{display:"flex",justifyContent:"center",marginBottom:20}}>
              <div style={{width:curShape.w,height:curShape.h,borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",backgroundColor:S3,border:`2px solid ${A}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
            ...
            {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:38,fontWeight:700,color:A,lineHeight:1,marginBottom:4,fontFamily:fnt,background:hex2rgba(A,0.08),border:`1px solid ${A}`,padding:"2px 12px",outline:"none",width:"100%",boxSizing:"border-box",textAlign:"center"}}/>:<div style={{fontSize:38,fontWeight:700,color:A,lineHeight:1,marginBottom:4,letterSpacing:"0.06em"}}>{data.personal.name}</div>}
            <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12,margin:"10px 0"}}>
              <div style={{height:1,flex:1,background:`linear-gradient(90deg,transparent,${A})`}}/>
              <span style={{color:A,fontSize:14}}>◆</span>
              <div style={{height:1,flex:1,background:`linear-gradient(90deg,${A},transparent)`}}/>
            </div>
            {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:12,color:CM,fontWeight:400,letterSpacing:"0.18em",fontFamily:fnt,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.3)}`,padding:"1px 8px",outline:"none",width:"60%",margin:"0 auto",boxSizing:"border-box",textAlign:"center"}}/>:<div style={{fontSize:12,color:CM,letterSpacing:"0.18em",textTransform:"uppercase"}}>{data.personal.subtitle}</div>}
            <E value={data.personal.bio} onChange={v=>setP({bio:v})} editing={editing} multiline rows={2} style={{fontSize:11,color:CB,lineHeight:1.7,display:"block",marginTop:10}}/>
            <div style={{display:"flex",flexWrap:"wrap",gap:16,marginTop:12,justifyContent:"center"}}>
              {(["email","phone","location","github","linkedin"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CVData["personal"]>)} style={{fontSize:10,color:CM,background:hex2rgba(A,0.07),border:`1px solid ${hex2rgba(A,0.3)}`,padding:"2px 8px",outline:"none",fontFamily:"inherit"}}/>:<span key={k} style={{fontSize:10,color:CM,letterSpacing:"0.04em"}}>{data.personal[k]}</span>:null)}
            </div>
          </div>
          <div style={{height:1,background:`linear-gradient(90deg,transparent 0%,${A} 30%,${A} 70%,transparent 100%)`,margin:"0 40px"}}/>
          <div style={{height:3,backgroundColor:BG}}/>
          {/* Body */}
          <div style={{display:"flex"}}>
            ...
            <div style={{width:240,flexShrink:0,backgroundColor:SBG,padding:"24px 18px 48px",borderLeft:`1px solid ${hex2rgba(A,0.3)}`}}>
              {[
