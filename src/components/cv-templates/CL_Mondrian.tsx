...
const DEFAULT_COLORS = { A:"#e63946", BG:"#ffffff", HD:"#fff9e0", S2:"#fff0a0", CT:"#000000", CB:"#1a1a1a", CM:"#555555" };
...
export default function CL_Mondrian(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clmond_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clmond_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clmond_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clmond_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f0f0f0",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"6px 6px 0 #000",overflow:"visible",position:"relative"}}>
