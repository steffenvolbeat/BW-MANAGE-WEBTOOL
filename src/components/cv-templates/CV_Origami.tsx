...
const DEFAULT_COLORS = { A:"#e8173e", BG:"#fafaf8", S2:"#f4f0eb", S3:"#ede8e0", SBG:"#ffffff", CT:"#1a1a1a", CB:"#2d2d2d", CM:"#888888" };
...
function E({ value, onChange, editing, multiline=false, style={} as React.CSSProperties, placeholder="...", rows=3 }:{ value:string; onChange:(v:string)=>void; editing:boolean; multiline?:boolean; style?:React.CSSProperties; placeholder?:string; rows?:number; }) {
  ...
  const s:React.CSSProperties = { ...style, background:hex2rgba(A,0.05), border:`1px dashed ${A}`, borderRadius:2, padding:"2px 4px", outline:"none", width:"100%", fontFamily:"inherit", fontSize:"inherit", color:"inherit", lineHeight:"inherit", fontWeight:"inherit", boxSizing:"border-box" };
...
function BulletList({ bullets, onChange, editing }:{ bullets:string[]; onChange:(b:string[])=>void; editing:boolean }) {
  const {A,CM,CB}=useContext(ColCtx);
  return (
    <ul style={{listStyle:"none",margin:0,padding:0}}>
      {bullets.map((b,i)=>(
        <li key={i} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
          <span style={{width:6,height:6,background:A,transform:"rotate(45deg)",flexShrink:0,marginTop:4,display:"inline-block"}}/>
          {editing?(
            <div style={{flex:1,display:"flex",gap:4}}>
              <input value={b} onChange={e=>{const n=[...bullets];n[i]=e.target.value;onChange(n);}} style={{flex:1,fontSize:11,color:CB,background:"#f9f6f1",border:`1px dashed ${A}44`,borderRadius:2,padding:"1px 4px",outline:"none",fontFamily:"inherit"}}/>
              <button type="button" onClick={()=>onChange(bullets.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#f87171",padding:0}}><XMarkIcon style={{width:11,height:11}}/></button>
            </div>
...
function TagList({ tags, onChange, editing }:{ tags:string[]; onChange:(t:string[])=>void; editing:boolean }) {
  const {A,BG,CT}=useContext(ColCtx);
  const [nv,setNv]=useState("");
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:5}}>
      {tags.map((t,i)=>(
        <div key={i} style={{background:hex2rgba(A,0.08),border:`1px solid ${hex2rgba(A,0.3)}`,color:CT,borderRadius:2,padding:"2px 10px",fontSize:10,display:"flex",alignItems:"center",gap:3}}>
      ...
      {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px dashed ${A}`,borderRadius:2,padding:"2px 5px",background:hex2rgba(A,0.05),outline:"none",color:CT,fontFamily:"inherit"}} placeholder="+ Neu"/>}
    </div>
  );
}
...
export default function CV_Origami() {
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("origami",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8e8e4",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>setEditing(e=>!e)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 16px",borderRadius:4,fontSize:13,fontWeight:600,cursor:"pointer",border:`2px solid ${editing?"#16a34a":A}`,backgroundColor:editing?"#16a34a":hex2rgba(A,0.1),color:editing?"white":A}}>
          {editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}
        </button>
        <button onClick={()=>{ const el=docRef.current; if(el){const h=el.scrollHeight;el.style.zoom=(h>1202?(0.934*1202/h):0.934).toFixed(5);} requestAnimationFrame(()=>requestAnimationFrame(()=>window.print())); }} style={{padding:"7px 16px",borderRadius:4,fontSize:13,cursor:"pointer",border:"2px solid #374151",backgroundColor:"#374151",color:"white",display:"flex",alignItems:"center",gap:6}}>
          <PrinterIcon style={{width:16,height:16}}/>Drucken
        </button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",borderRadius:4,fontSize:13,cursor:"pointer",border:`2px solid ${showDesign?A:"#ccc"}`,backgroundColor:"transparent",color:showDesign?A:"#888"}}>🎨 Design</button>
        <button onClick={()=>{resetStorage();setData(JSON.parse(JSON.stringify(DEFAULT_CV_DATA)));setPhotoSrc("");setFontKey("nunito");setSizeKey("md");setPhotoShapeKey("square");setClrs(DEFAULT_COLORS);setShowDesign(false);}} style={{padding:"7px 16px",borderRadius:4,fontSize:13,cursor:"pointer",border:"2px solid #ccc",backgroundColor:"transparent",color:"#888",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(
          <div style={{width:"100%",background:"#fff",border:"1px solid #ddd",borderRadius:6,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
            <div><div style={{fontSize:10,fontWeight:700,color:"#999",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Font</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{FONTS.map(f=><button key={f.key} onClick={()=>setFontKey(f.key)} style={{padding:"3px 9px",borderRadius:3,border:`1px solid ${fontKey===f.key?A:"#ddd"}`,background:fontKey===f.key?hex2rgba(A,0.1):"transparent",color:fontKey===f.key?A:"#666",fontSize:11,cursor:"pointer",fontFamily:f.family}}>{f.label}</button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:700,color:"#999",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Größe</div><div style={{display:"flex",gap:5}}>{FONT_SIZES.map(s=><button key={s.key} onClick={()=>setSizeKey(s.key)} style={{padding:"3px 12px",borderRadius:3,border:`1px solid ${sizeKey===s.key?A:"#ddd"}`,background:sizeKey===s.key?hex2rgba(A,0.1):"transparent",color:sizeKey===s.key?A:"#666",fontSize:11,cursor:"pointer"}}>{s.label}</button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:700,color:"#999",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Foto</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{PHOTO_SHAPES.map(s=><button key={s.key} onClick={()=>setPhotoShapeKey(s.key)} style={{padding:"4px 8px",borderRadius:3,border:`2px solid ${photoShapeKey===s.key?A:"#ddd"}`,background:photoShapeKey===s.key?hex2rgba(A,0.1):"transparent",cursor:"pointer",fontSize:10,color:photoShapeKey===s.key?A:"#666"}}>{s.label}</button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:700,color:"#999",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Akzent"},{k:"BG" as const,l:"BG"},{k:"S2" as const,l:"Sidebar"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Gedimmt"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,borderRadius:3,border:"1px solid #ddd",cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:"#999"}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",borderRadius:3,fontSize:11,border:"1px solid #ddd",background:"transparent",color:"#888",cursor:"pointer",alignSelf:"center"}}>↺</button></div></div>
          </div>
        )}
      </div>

      <div ref={docRef} className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,overflow:"visible",fontFamily:fnt,position:"relative",boxShadow:"0 4px 40px rgba(0,0,0,0.15)"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale,position:"relative"}}>
          {/* Origami top-left triangle accent */}
          <div style={{position:"absolute",top:0,right:0,width:0,height:0,borderStyle:"solid",borderWidth:"0 80px 80px 0",borderColor:`transparent ${A} transparent transparent`,zIndex:1}}/>
          {/* Header */}
          <div style={{background:S2,padding:"32px 40px 24px",borderBottom:`4px solid ${A}`,position:"relative",overflow:"hidden"}}>
            {/* diagonal fold line */}
            <div style={{position:"absolute",bottom:-1,left:0,right:0,height:8,background:`linear-gradient(135deg,${A} 0%,${A} 50%,transparent 50%,transparent 100%)`}}/>
            <div style={{display:"flex",gap:28,alignItems:"center"}}>
              <div style={{position:"relative",width:curShape.w,height:curShape.h,flexShrink:0,cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
                <div style={{width:"100%",height:"100%",borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",backgroundColor:S3,border:`3px solid ${A}`,display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:32,fontWeight:900,color:CT,marginBottom:4,fontFamily:"inherit",background:hex2rgba(A,0.05),border:`1px dashed ${A}`,borderRadius:2,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:32,fontWeight:900,color:CT,marginBottom:4,letterSpacing:"-0.5px"}}>{data.personal.name}</div>}
                {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:14,color:A,marginBottom:8,fontFamily:"inherit",background:hex2rgba(A,0.05),border:`1px dashed ${A}`,borderRadius:2,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:14,fontWeight:600,color:A,marginBottom:8,letterSpacing:"0.05em"}}>{data.personal.subtitle}</div>}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:8,marginTop:10}}>
                  {(["email","phone","location","github","linkedin","website"] as const).map(k=>editing
                    ?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CVData["personal"]>)} style={{fontSize:10,color:CT,background:hex2rgba(A,0.05),border:`1px dashed ${A}44`,borderRadius:2,padding:"2px 8px",outline:"none",fontFamily:"inherit"}}/>
                    :data.personal[k]?<span key={k} style={{fontSize:10,color:CM,background:S3,padding:"2px 10px",borderRadius:2,border:`1px solid ${A}22`}}>{data.personal[k]}</span>:null
                  )}
                </div>
              </div>
            </div>
                ...
                {data.languages.map((l,i)=>(
                  <div key={i} style={{marginBottom:8}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:3}}>
                      {editing?<input value={l.language} onChange={e=>{const n=[...data.languages];n[i]={...n[i],language:e.target.value};setData(d=>({...d,languages:n}));}} style={{fontSize:11,color:CT,background:hex2rgba(A,0.05),border:`1px dashed ${A}44`,borderRadius:2,padding:"1px 4px",outline:"none",width:80,fontFamily:"inherit"}}/>
                        :<span style={{fontSize:11,color:CT,fontWeight:600}}>{l.language}</span>}
                      {editing?<input value={l.level} onChange={e=>{const n=[...data.languages];n[i]={...n[i],level:e.target.value};setData(d=>({...d,languages:n}));}} style={{fontSize:10,color:CM,background:hex2rgba(A,0.05),border:`1px dashed ${A}44`,borderRadius:2,padding:"1px 4px",outline:"none",width:60,fontFamily:"inherit"}}/>
                        :<span style={{fontSize:10,color:CM}}>{l.level}</span>}
                    </div>
                    <div style={{height:3,background:`${A}22`,borderRadius:2}}><div style={{height:"100%",width:l.level==="Muttersprache"?"100%":l.level==="Verhandlungssicher"?"85%":l.level==="Fließend"?"70%":l.level==="Grundkenntnisse"?"40%":"60%",background:A,borderRadius:2}}/></div>
                  </div>
                ))}
              </div>
              <div style={{marginBottom:20}}>
            ...
            <div style={{flex:1,padding:"22px 28px"}}>
              <div style={{marginBottom:20}}>
                ...
                {data.experience.map(ex=>(
                  <div key={ex.id} style={{marginBottom:16,paddingLeft:12,borderLeft:`2px solid ${A}44`,position:"relative"}}>
                    <div style={{position:"absolute",left:-5,top:4,width:8,height:8,background:A,borderRadius:0,transform:"rotate(45deg)"}}/>
              ...
              <div style={{marginBottom:20}}>
                <SecH title="Projekte"/>
                {data.projects.map(p=>(
                  <div key={p.id} style={{marginBottom:14,paddingLeft:12,borderLeft:`2px solid ${A}44`,position:"relative"}}>
                    <div style={{position:"absolute",left:-5,top:4,width:8,height:8,background:`${A}44`,borderRadius:0,transform:"rotate(45deg)"}}/>
