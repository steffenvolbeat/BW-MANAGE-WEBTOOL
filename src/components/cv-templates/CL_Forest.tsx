"use client";
// ─── Anschreiben Template: Forest (Grün-natur) ────────────────────────────
...
const DEFAULT_COLORS = { A:"#22c55e", BG:"#14532d", HD:"#2e8651", S2:"#1a3a20", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Forest() {
  const [data, setData] = usePersistentCLState<CLData>('cl_forest_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_forest_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_forest_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_forest_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#111", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
        ...
        .clfst-doc, .clfst-doc * { font-family: ${fnt} !important; }
        @media print {
          @page { size: A4 portrait; margin: 0; }
          *, *::before, *::after { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
          body * { visibility: hidden !important; }
          .clfst-doc, .clfst-doc * { visibility: visible !important; }
          .clfst-doc { position: absolute !important; top: 0 !important; left: 0 !important; width: 850px !important; min-height: 1202px !important; overflow: hidden !important; zoom: 0.934 !important; box-shadow: none !important; margin: 0 !important; }
          .clfst-zoom { zoom: 1 !important; width: 100% !important; }
          .clfst-ctrl { display: none !important; }
        }
      `}</style>
      ...
      <div className="clfst-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
