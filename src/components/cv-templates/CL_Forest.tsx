"use client";
// ─── Anschreiben Template: Forest (Dunkelgrün) ────────────────────────────────
...
const A = "#22c55e"; const DARK = "#14532d"; const SBG = "#f0fdf4";
...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(255,255,255,0.1)" : "rgba(34,197,94,0.06)", border: `1px dashed ${inv ? "rgba(255,255,255,0.4)" : `${A}55`}`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Forest() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#052e16", padding: "24px 16px" }}>
      <style>{`${curFont.gf ? `@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');` : ""} .clfst-doc,.clfst-doc * { font-family: ${fnt} !important; } @media print { @page{size:A4 portrait;margin:0} body *{visibility:hidden!important} .clfst-doc,.clfst-doc *{visibility:visible!important} .clfst-doc{position:absolute!important;top:0!important;left:0!important;width:210mm!important;box-shadow:none!important;margin:0!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important} .clfst-zoom{zoom:1!important;width:100%!important} .clfst-ctrl{display:none!important} }`}</style>
      <div className="clfst-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        <button onClick={() => setEditing(e => !e)} style={{ padding: "7px 16px", borderRadius: 8, fontSize: 13, fontWeight: 600, cursor: "pointer", border: "none", backgroundColor: editing ? "#16a34a" : DARK, color: "white", display: "flex", alignItems: "center", gap: 6 }}>{editing ? <CheckIcon style={{ width: 16, height: 16 }} /> : <PencilSquareIcon style={{ width: 16, height: 16 }} />}{editing ? "Fertig" : "Bearbeiten"}</button>
        ...
        {showDesign && <div style={{ width: "100%", background: "#0a2e0a", border: `1px solid ${A}44`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#166534", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#166534"}`, background: fontKey === f.key ? "rgba(34,197,94,0.12)" : "transparent", color: fontKey === f.key ? A : "#6b7280", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#166534", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Größe</div><div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#166534"}`, background: sizeKey === s.key ? "rgba(34,197,94,0.12)" : "transparent", color: sizeKey === s.key ? A : "#6b7280", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div></div>
        </div>}
      </div>
      <div className="clfst-doc" style={{ width: 850, margin: "0 auto", backgroundColor: "white", boxShadow: `0 8px 40px rgba(34,197,94,0.2)`, overflow: "hidden" }}>
        <div className="clfst-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          {/* Header split: dark sidebar + white */}
          <div style={{ display: "flex", background: DARK }}>
            <div style={{ width: 220, padding: "28px 20px", flexShrink: 0 }}>
              <E value={data.personal.name} onChange={v => setP({ name: v })} editing={editing} inv style={{ fontSize: 20, fontWeight: 900, color: "white", display: "block", marginBottom: 4 }} />
              <E value={data.personal.subtitle} onChange={v => setP({ subtitle: v })} editing={editing} inv style={{ fontSize: 10, color: `${A}cc`, display: "block", marginBottom: 14 }} />
              {[data.personal.email, data.personal.phone, data.personal.location].map((v, i) => <E key={i} value={v} onChange={nv => setP([{ email: nv }, { phone: nv }, { location: nv }][i])} editing={editing} inv style={{ fontSize: 9, color: "rgba(255,255,255,0.6)", display: "block", marginBottom: 3 }} />)}
            </div>
            <div style={{ flex: 1, background: A, display: "flex", alignItems: "center", padding: "0 20px" }}>
              <div style={{ width: 3, height: 60, background: "rgba(255,255,255,0.3)", borderRadius: 2, marginRight: 16 }} />
              <span style={{ fontSize: 12, color: "white", fontWeight: 600, fontStyle: "italic", opacity: 0.85 }}>Bewerbungsschreiben</span>
            </div>
          </div>
          <div style={{ padding: "32px 48px 48px", color: "#374151" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
              <div style={{ borderLeft: `3px solid ${A}`, paddingLeft: 12 }}>{[data.recipient.company, data.recipient.street, data.recipient.cityZip].map((v, i) => <E key={i} value={v} onChange={nv => setR([{ company: nv }, { street: nv }, { cityZip: nv }][i])} editing={editing} style={{ fontSize: 11, color: "#374151", display: "block", lineHeight: 1.6 }} />)}</div>
              <E value={data.date} onChange={v => setData(d => ({ ...d, date: v }))} editing={editing} style={{ fontSize: 11, color: "#9ca3af", fontStyle: "italic" }} />
            </div>
            <div style={{ marginBottom: 20, paddingBottom: 12, borderBottom: `2px solid ${A}44` }}>
              <E value={data.subject} onChange={v => setData(d => ({ ...d, subject: v }))} editing={editing} style={{ fontSize: 13, fontWeight: 700, color: DARK }} />
            </div>
            <E value={data.salutation} onChange={v => setData(d => ({ ...d, salutation: v }))} editing={editing} style={{ fontSize: 12, color: "#374151", display: "block", marginBottom: 14 }} />
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ width: 100, height: 2, backgroundColor: A, borderRadius: 1, marginBottom: 8 }} />
