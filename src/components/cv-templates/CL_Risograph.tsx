"use client";
// ─── Anschreiben Template: Risograph ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#e8416e", BG:"#fdf4e3", HD:"#f8ead8", S2:"#f0e0c8", CT:"#2a1018", CB:"#3a1a28", CM:"#a06878" };
...
export default function CL_Risograph(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clriso_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clriso_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clriso_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clriso_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f0e4d0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(232,65,110,0.12)",overflow:"visible",position:"relative"}}>
