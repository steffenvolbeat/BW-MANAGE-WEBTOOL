"use client";
// ─── Anschreiben Template: Dusk (Twilight Gradient, Amber→Indigo) ────────────
...
const DEFAULT_COLORS = { A:"#e07840", BG:"#100c20", HD:"#1a1030", S2:"#160e28", CT:"#fff0e0", CB:"#ffd8c0", CM:"#8068a0" };
...
const PFX="cldsk";
const TWILIGHT="#6040c0"; const STAR="#ffd080";
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=4}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.08),border:`1px dashed ${A}55`,padding:"2px 5px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
export default function CL_Dusk(){
  const [data,setData]=usePersistentCLState<CLData>('cl_cldsk_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_cldsk_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_cldsk_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_cldsk_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#080614",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${A}`,background:editing?A:"transparent",color:editing?"#100c20":A,display:"flex",alignItems:"center",gap:6}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${TWILIGHT}`,background:"transparent",color:TWILIGHT,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
        <button onClick={()=>{setData(JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));setClrs(DEFAULT_COLORS);}} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:"2px solid #333",background:"transparent",color:"#555",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:14,height:14}}/>Reset</button>
      </div>

      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 0 60px ${hex2rgba(A,0.15)}, 0 0 100px ${hex2rgba(TWILIGHT,0.1)}`,overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{background:`linear-gradient(180deg, ${A}33 0%, ${TWILIGHT}44 100%), ${HD}`,padding:"32px 48px 22px",borderBottom:`1px solid ${A}44`,position:"relative",overflow:"hidden"}}>
            {/* Stars */}
            {[[80,20],[200,10],[400,30],[550,15],[700,25],[750,8]].map(([x,y],i)=>(
              <div key={i} style={{position:"absolute",left:x,top:y,width:2,height:2,borderRadius:"50%",background:STAR,opacity:0.7}}/>
            ))}
            <div style={{position:"relative"}}>
              <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:28,fontWeight:900,color:CT,display:"block",textShadow:`0 0 16px ${A}`}}/>
              <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:11,color:A,display:"block",marginTop:4,letterSpacing:"0.08em"}}/>
          ...
          <div style={{padding:"28px 48px 48px"}}>
            ...
            <div style={{marginBottom:16,padding:"8px 14px",background:`linear-gradient(90deg, ${hex2rgba(A,0.12)}, transparent)`,borderLeft:`3px solid ${A}`}}>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{height:1,background:`linear-gradient(90deg, ${A}, ${TWILIGHT})`,marginBottom:8}}/>
              <E value={data.signatureName} onChange={v=>setData(d=>({...d,signatureName:v}))} editing={editing} style={{fontSize:13,fontWeight:700,color:CT,textShadow:`0 0 8px ${A}`}}/>
            </div>
          </div>
        </div>
      </div>
