"use client";
// ─── Anschreiben Template: Manuscript ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#8b2252", BG:"#fdf6e3", HD:"#f8edd8", S2:"#f2e4cc", CT:"#2a0a18", CB:"#3a1020", CM:"#886070" };
...
export default function CL_Manuscript(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clmanu_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clmanu_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clmanu_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clmanu_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8d8c0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(139,34,82,0.15)",overflow:"visible",position:"relative"}}>
