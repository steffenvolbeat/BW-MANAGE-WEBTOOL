"use client";
// ─── CV Template: Amber (Warm Amber/Whisky, Cream Paper) ─────────────────────
...
const DEFAULT_COLORS = { A:"#c47a20", BG:"#faf4e8", S2:"#f5edd8", S3:"#ede4c8", SBG:"#fdf8f0", CT:"#2a1f0a", CB:"#3a2e18", CM:"#8a7050" };
...
const PFX = "amb";
const AMBER="#c47a20"; const WHISKY="#a05a10"; const GOLD="#e0a030"; const WARM="#f0d090";
...
function TagList({ tags, onChange, editing }:{ tags:string[]; onChange:(t:string[])=>void; editing:boolean }) {
  ...
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
      {tags.map((t,i)=>(
        <div key={i} style={{background:hex2rgba(AMBER,0.12),border:`1px solid ${AMBER}77`,color:WHISKY,padding:"2px 9px",fontSize:10,fontWeight:600,display:"flex",alignItems:"center",gap:3}}>
      ...
      {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1.5px dashed ${AMBER}`,padding:"2px 6px",background:hex2rgba(AMBER,0.06),outline:"none",color:AMBER,fontFamily:"inherit"}} placeholder="+ Neu"/>}
    </div>
  );
}
...
function SecH({ title, color=AMBER }:{ title:string; color?:string }) {
  return (
    <div style={{marginBottom:10,paddingBottom:6,borderBottom:`1px solid ${color}66`}}>
      <span style={{fontSize:11,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase" as const,color}}>{title}</span>
    </div>
  );
}

export default function CV_Amber() {
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("amber",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f0e8d4",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>setEditing(e=>!e)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${editing?"#2f7d3e":AMBER}`,backgroundColor:editing?"#2f7d3e":hex2rgba(AMBER,0.08),color:editing?"white":AMBER}}>
          {editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}
        </button>
        <button onClick={()=>{ const el=docRef.current; if(el){const h=el.scrollHeight;el.style.zoom=(h>1202?(0.934*1202/h):0.934).toFixed(5);} requestAnimationFrame(()=>requestAnimationFrame(()=>window.print())); }} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${WHISKY}`,backgroundColor:hex2rgba(WHISKY,0.08),color:WHISKY,display:"flex",alignItems:"center",gap:6}}>
          <PrinterIcon style={{width:16,height:16}}/>Drucken
        </button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${showDesign?AMBER:"#aaa"}`,backgroundColor:"transparent",color:showDesign?AMBER:"#777"}}>🎨</button>
      ...
      <div ref={docRef} className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,overflow:"visible",fontFamily:fnt,boxShadow:"0 4px 20px rgba(100,60,0,0.15)"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          {/* Amber header with warm gradient */}
          <div style={{background:`linear-gradient(135deg, ${AMBER} 0%, ${WHISKY} 100%)`,padding:"28px 36px 22px",position:"relative",overflow:"hidden"}}>
            {/* Warm glow overlay */}
            <div style={{position:"absolute",top:0,right:0,width:200,height:"100%",background:`radial-gradient(ellipse at right top, ${hex2rgba(GOLD,0.3)} 0%, transparent 70%)`,pointerEvents:"none"}}/>
            <div style={{display:"flex",gap:24,alignItems:"center"}}>
              <div style={{flexShrink:0,position:"relative"}}>
                <div style={{width:curShape.w,height:curShape.h,borderRadius:curShape.br,overflow:"hidden",backgroundColor:S3,border:`3px solid ${WARM}`,cursor:editing?"pointer":"default",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`0 0 15px rgba(0,0,0,0.3)`}} onClick={()=>editing&&photoInputRef.current?.click()}>
                  {photoSrc?<img src={photoSrc} alt="Foto" style={{width:"100%",height:"100%",objectFit:"cover"}}/>:<span style={{fontSize:10,color:WARM,textAlign:"center"}}>{editing?"📷":"Foto"}</span>}
                </div>
              ...
              <div style={{flex:1}}>
                <div style={{marginBottom:4}}>{editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{fontSize:28,fontWeight:700,color:"#fff",fontFamily:"inherit",background:"rgba(255,255,255,0.15)",border:"1.5px dashed rgba(255,255,255,0.5)",padding:"2px 8px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<span style={{fontSize:28,fontWeight:700,color:"#fff"}}>{data.personal.name}</span>}</div>
          ...
          <div style={{display:"flex"}}>
            <div style={{flex:1,padding:"18px 22px"}}>
              <E value={data.personal.bio} onChange={v=>setP({bio:v})} editing={editing} multiline rows={2} style={{fontSize:11,color:CM,lineHeight:1.7,display:"block",marginBottom:14,padding:"8px 10px",background:S2,borderLeft:`3px solid ${AMBER}66`}}/>
              ...
              <div><SecH title="Projekte" color={WHISKY}/>
                {data.projects.map(p=>(
                  <div key={p.id} style={{marginBottom:10,paddingLeft:10,borderLeft:`2px solid ${WHISKY}55`}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2}}>
                      <E value={p.title} onChange={v=>updProj(p.id,{title:v})} editing={editing} style={{fontSize:12,fontWeight:700,color:WHISKY}}/>
                ...
                {editing&&<button type="button" onClick={()=>setData(d=>({...d,projects:[...d.projects,{id:uid(),title:"",period:"",bullets:[],link:""}]}))} style={{fontSize:11,color:WHISKY,background:"none",border:`1.5px solid ${WHISKY}55`,cursor:"pointer",padding:"3px 10px",display:"flex",alignItems:"center",gap:4}}><PlusIcon style={{width:12,height:12}}/>Hinzufügen</button>}
              </div>
            </div>
            <div style={{width:210,background:S2,borderLeft:`1px solid ${AMBER}44`,padding:"18px 14px"}}>
              ...
              <div style={{marginBottom:14}}>
                <SecH title="Ausbildung" color={AMBER}/>
                {data.education.map(e=>(
                  <div key={e.id} style={{marginBottom:10,padding:"6px",background:BG,border:`1px solid ${AMBER}44`}}>
                    <E value={e.degree} onChange={v=>updEdu(e.id,{degree:v})} editing={editing} style={{fontSize:11,fontWeight:700,color:CT,display:"block"}}/>
                    <E value={e.institution} onChange={v=>updEdu(e.id,{institution:v})} editing={editing} style={{fontSize:10,color:AMBER,display:"block"}}/>
                ...
                {editing&&<button type="button" onClick={()=>setData(d=>({...d,education:[...d.education,{id:uid(),degree:"",institution:"",period:"",location:"",type:"Kurse / Inhalte",bullets:[]}] }))} style={{fontSize:11,color:AMBER,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:3}}><PlusIcon style={{width:12,height:12}}/>+</button>}
              </div>
              <div><SecH title="Sprachen" color={AMBER}/>
                {data.languages.map((l,i)=>(
                  <div key={i} style={{marginBottom:6,display:"flex",justifyContent:"space-between"}}>
                    ...
                    <span style={{fontSize:9,background:hex2rgba(AMBER,0.15),color:WHISKY,padding:"1px 6px"}}>{l.level}</span>
                  </div>
                ))}
              </div>
            </div>
