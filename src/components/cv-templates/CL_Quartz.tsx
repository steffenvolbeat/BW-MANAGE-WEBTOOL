"use client";
// ─── Anschreiben Template: Quartz (Crystal, Dark, Gemstone) ─────────────────
...
const DEFAULT_COLORS = { A:"#a855f7", BG:"#0f0618", HD:"#17082a", S2:"#1c0e30", CT:"#f5e8ff", CB:"#ddc8ff", CM:"#7040a0" };
...
const PFX="clqtz";
const ROSE="#e887b0"; const ICE="#c8e8ff";
...
export default function CL_Quartz(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clqtz_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clqtz_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clqtz_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clqtz_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#080310",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${A}`,background:editing?A:"transparent",color:editing?"#0f0618":A,display:"flex",alignItems:"center",gap:6,textShadow:editing?"none":`0 0 6px ${A}`}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 0 50px ${hex2rgba(A,0.2)}, 0 0 100px ${hex2rgba(ROSE,0.08)}`,overflow:"visible",border:`1px solid ${A}33`}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{background:`linear-gradient(135deg, ${HD} 0%, ${hex2rgba(ROSE,0.15)} 50%, ${hex2rgba(A,0.2)} 100%)`,padding:"32px 48px 22px",borderBottom:`2px solid ${A}55`,clipPath:"polygon(0 0,100% 0,100% 88%,95% 100%,0 100%)"}}>
          ...
          <div style={{padding:"30px 48px 48px"}}>
            ...
            <div style={{marginBottom:16,padding:"8px 14px",background:`linear-gradient(90deg, ${hex2rgba(A,0.12)}, transparent)`,borderLeft:`3px solid ${ROSE}`,boxShadow:`-4px 0 10px ${hex2rgba(ROSE,0.2)}`}}>
              <E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:13,fontWeight:700,color:ROSE,textShadow:`0 0 8px ${ROSE}`}}/>
            </div>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{height:1,background:`linear-gradient(90deg, ${ROSE}, ${A}, ${ICE})`,marginBottom:8,boxShadow:`0 0 6px ${A}`}}/>
