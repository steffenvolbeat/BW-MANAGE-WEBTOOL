"use client";
// ─── Anschreiben Template: Arctic (Hellblau/Eisig) ────────────────────────────
...
const A = "#0ea5e9"; const A2 = "#38bdf8"; const LIGHT = "#f0f9ff";
...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(255,255,255,0.12)" : "rgba(14,165,233,0.06)", border: `1px dashed ${inv ? "rgba(255,255,255,0.4)" : `${A}55`}`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Arctic() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: LIGHT, padding: "24px 16px" }}>
      <style>{`${curFont.gf ? `@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');` : ""} .clarc-doc,.clarc-doc * { font-family: ${fnt} !important; } @media print { @page{size:A4 portrait;margin:0} body *{visibility:hidden!important} .clarc-doc,.clarc-doc *{visibility:visible!important} .clarc-doc{position:absolute!important;top:0!important;left:0!important;width:210mm!important;box-shadow:none!important;margin:0!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important} .clarc-zoom{zoom:1!important;width:100%!important} .clarc-ctrl{display:none!important} }`}</style>
      <div className="clarc-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        <button onClick={() => setShowDesign(v => !v)} style={{ padding: "7px 16px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: `1px solid ${showDesign ? A : "#e5e7eb"}`, backgroundColor: showDesign ? LIGHT : "white", color: showDesign ? A : "#374151" }}>🎨 Design</button>
        <button onClick={() => { if (window.confirm("Zurücksetzen?")) setData(JSON.parse(JSON.stringify(DEFAULT_CL_DATA))); }} style={{ padding: "7px 16px", borderRadius: 8, fontSize: 13, cursor: "pointer", border: "1px solid #e5e7eb", backgroundColor: "white", color: "#374151", display: "flex", alignItems: "center", gap: 6 }}><XMarkIcon style={{ width: 16, height: 16 }} />Reset</button>
        {showDesign && <div style={{ width: "100%", background: "white", border: `1px solid #bae6fd`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#0ea5e9", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#e5e7eb"}`, background: fontKey === f.key ? LIGHT : "white", color: fontKey === f.key ? A : "#374151", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#0ea5e9", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Größe</div><div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#e5e7eb"}`, background: sizeKey === s.key ? LIGHT : "white", color: sizeKey === s.key ? A : "#374151", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div></div>
        </div>}
      </div>
      <div className="clarc-doc" style={{ width: 850, margin: "0 auto", backgroundColor: "white", boxShadow: `0 4px 24px rgba(14,165,233,0.15)`, overflow: "hidden", border: `1px solid #bae6fd` }}>
        <div className="clarc-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          <div style={{ height: 4, background: `linear-gradient(90deg,${A},${A2})` }} />
          <div style={{ background: `linear-gradient(135deg,${A} 0%,${A2} 100%)`, padding: "28px 48px 22px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <div><E value={data.personal.name} onChange={v => setP({ name: v })} editing={editing} inv style={{ fontSize: 28, fontWeight: 900, color: "white", display: "block", marginBottom: 3 }} /><E value={data.personal.subtitle} onChange={v => setP({ subtitle: v })} editing={editing} inv style={{ fontSize: 11, color: "rgba(255,255,255,0.85)", display: "block" }} /></div>
              <div style={{ textAlign: "right" }}>{[data.personal.email, data.personal.phone, data.personal.location].map((v, i) => <E key={i} value={v} onChange={nv => setP([{ email: nv }, { phone: nv }, { location: nv }][i])} editing={editing} inv style={{ fontSize: 10, color: "rgba(255,255,255,0.8)", display: "block", marginBottom: 2 }} />)}</div>
            </div>
          </div>
          <div style={{ padding: "32px 48px 48px", color: "#374151" }}>
            ...
            <div style={{ marginBottom: 20, paddingBottom: 12, borderBottom: `2px solid ${A2}` }}>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ width: 80, height: 2, background: `linear-gradient(90deg,${A},${A2})`, borderRadius: 1, marginBottom: 8 }} />
              <E value={data.signatureName} onChange={v => setData(d => ({ ...d, signatureName: v }))} editing={editing} style={{ fontSize: 12, fontWeight: 700, color: A }} />
            </div>
          </div>
        </div>
      </div>
