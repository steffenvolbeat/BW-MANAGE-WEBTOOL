...
const DEFAULT_COLORS = { A:"#f97316", BG:"#fefce8", HD:"#fef9c3", S2:"#fef08a", CT:"#1c1200", CB:"#2a1800", CM:"#a06000" };
...
export default function CL_Atomic(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clatom_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clatom_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clatom_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clatom_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f5e8c0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(249,115,22,0.2)",overflow:"visible",position:"relative"}}>
