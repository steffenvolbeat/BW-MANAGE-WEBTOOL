"use client";
// ─── Anschreiben Template: Parchment (Altes Pergament) ─────────────────────
...
const DEFAULT_COLORS = { A:"#6b3a2a", BG:"#fef3e2", HD:"#f5e6c8", S2:"#f5e6c8", CT:"#3a1a0a", CB:"#5a3020", CM:"#9a7a5a" };
...
export default function CL_Parchment() {
  const [data,setData]=usePersistentCLState<CLData>("cl_parchment_data",JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState("cl_parchment_font","georgia");
  const [sizeKey,setSizeKey]=usePersistentCLState("cl_parchment_size","md");
  const [showDesign,setShowDesign]=useState(false);
  const [clrs,setClrs]=usePersistentCLState("cl_parchment_colors",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#d4b896",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
        ...
        .clpar-doc,.clpar-doc *{font-family:${fnt}!important;}
        @media print{
          @page{size:A4 portrait;margin:0;}
          *,*::before,*::after{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
          body *{visibility:hidden!important;}
          .clpar-doc,.clpar-doc *{visibility:visible!important;}
          .clpar-doc{position:absolute!important;top:0!important;left:0!important;width:850px!important;min-height:1202px!important;overflow:visible!important;zoom:0.934!important;box-shadow:none!important;margin:0!important;}
          .clpar-zoom{zoom:1!important;width:100%!important;}
          .clpar-ctrl{display:none!important;}
        }
      `}</style>
      <div className="clpar-ctrl" style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,fontWeight:600,cursor:"pointer",border:`1px solid ${editing?"#16a34a":A}`,borderRadius:3,backgroundColor:editing?"#16a34a":hex2rgba(A,0.08),color:editing?"white":A,display:"flex",alignItems:"center",gap:6}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`1px solid ${A}`,borderRadius:3,backgroundColor:A,color:"#fef3e2",display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`1px solid ${showDesign?A:"#b09070"}`,borderRadius:3,backgroundColor:"transparent",color:showDesign?A:"#8a6040"}}>🎨 Design</button>
        <button onClick={doReset} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:"1px solid #b09070",borderRadius:3,backgroundColor:"transparent",color:"#8a6040",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(<div style={{width:"100%",background:"#f5e6c8",border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:4,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
          ...
          <div><div style={{fontSize:10,fontWeight:700,color:CM,marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Braun"},{k:"BG" as const,l:"BG"},{k:"HD" as const,l:"Header"},{k:"S2" as const,l:"Panel"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Meta"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,border:`1px solid ${hex2rgba(A,0.3)}`,cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:CM}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",fontSize:11,border:`1px solid ${hex2rgba(A,0.3)}`,background:"transparent",color:CM,cursor:"pointer",alignSelf:"center"}}>↺</button></div></div>
        </div>)}
      </div>
      <div className="clpar-doc" style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:"0 3px 20px rgba(100,50,20,0.25)",fontFamily:fnt,overflow:"hidden"}}>
        <div className="clpar-zoom" style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{height:6,background:`repeating-linear-gradient(90deg,${A} 0,${A} 4px,${hex2rgba(A,0.3)} 4px,${hex2rgba(A,0.3)} 8px)`}}/>
          <div style={{backgroundColor:HD,padding:"44px 64px 32px",textAlign:"center",borderBottom:`1px solid ${hex2rgba(A,0.2)}`}}>
            <div style={{fontSize:11,color:A,letterSpacing:"0.3em",fontStyle:"italic",marginBottom:10}}>❧ Bewerbungsschreiben ❧</div>
            ...
            <div style={{height:1,background:`linear-gradient(90deg,transparent,${A},transparent)`,margin:"12px 0 10px"}}/>
            <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:12,color:CM,display:"block",fontStyle:"italic"}}/>
            <div style={{display:"flex",gap:28,flexWrap:"wrap",marginTop:14,justifyContent:"center"}}>{(["email","phone","location","website","linkedin","github"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CLData["personal"]>)} style={{fontSize:10,color:CM,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.2)}`,padding:"2px 8px",outline:"none",fontFamily:"inherit",borderRadius:2}}/>:<span key={k} style={{fontSize:10,color:CM}}>{data.personal[k]}</span>:null)}</div>
          </div>
          <div style={{padding:"36px 64px 52px",color:CB}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:32,gap:16,alignItems:"flex-start"}}>
              <div style={{flex:1}}>{(["company","street","cityZip","country"] as const).map(k=><E key={k} value={data.recipient[k]??""} onChange={v=>setR({[k]:v} as Partial<CLData["recipient"]>)} editing={editing} style={{fontSize:12,color:CB,display:"block",lineHeight:1.8}}/>)}</div>
              <E value={data.date} onChange={v=>setData(d=>({...d,date:v}))} editing={editing} style={{fontSize:11,color:CM,fontStyle:"italic"}}/>
            </div>
            <div style={{marginBottom:28,paddingBottom:14,borderBottom:`1px solid ${hex2rgba(A,0.25)}`}}><E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:15,fontWeight:700,color:CT}}/></div>
            ...
            <div style={{marginTop:44}}><E value={data.closing} onChange={v=>setData(d=>({...d,closing:v}))} editing={editing} style={{fontSize:12,color:CB,display:"block",marginBottom:48}}/><div style={{width:130,height:1,backgroundColor:hex2rgba(A,0.4),marginBottom:10}}/><E value={data.signatureName} onChange={v=>setData(d=>({...d,signatureName:v}))} editing={editing} style={{fontSize:17,fontWeight:700,color:CT,fontStyle:"italic"}}/></div>
          </div>
          <div style={{height:6,background:`repeating-linear-gradient(90deg,${A} 0,${A} 4px,${hex2rgba(A,0.3)} 4px,${hex2rgba(A,0.3)} 8px)`}}/>
        </div>
      </div>
