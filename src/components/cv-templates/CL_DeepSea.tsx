...
const DEFAULT_COLORS = { A:"#00e5ff", BG:"#000d1a", HD:"#001428", S2:"#001e38", CT:"#e0f8ff", CB:"#a0d8ee", CM:"#406888" };
...
export default function CL_DeepSea(){
  const [data,setData]=usePersistentCLState<CLData>('cl_cldsea_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_cldsea_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_cldsea_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_cldsea_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#000810",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 60px rgba(0,229,255,0.15)",overflow:"visible",position:"relative"}}>
