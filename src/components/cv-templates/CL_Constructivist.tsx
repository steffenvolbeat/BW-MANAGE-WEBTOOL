"use client";
// ─── Anschreiben Template: Constructivist ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#dc2626", BG:"#ffffff", HD:"#f0f0f0", S2:"#e0e0e0", CT:"#000000", CB:"#1a1a1a", CM:"#666666" };
...
export default function CL_Constructivist(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clcstr_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clcstr_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clcstr_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clcstr_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e0e0e0",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"6px 6px 0 #dc2626",overflow:"visible",position:"relative"}}>
