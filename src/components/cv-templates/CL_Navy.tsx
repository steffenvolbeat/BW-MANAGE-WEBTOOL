...
const DEFAULT_COLORS = { A:"#3b82f6", BG:"#1e3a5f", HD:"#3c6397", S2:"#1e40af", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Navy() {
  const [data, setData] = usePersistentCLState<CLData>('cl_navy_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_navy_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_navy_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_navy_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#1e3a5f", padding: "24px 16px", fontFamily: fnt }}>
      <style>{`
        ...
        .clnav-doc, .clnav-doc * { font-family: ${fnt} !important; }
        @media print {
          @page { size: A4 portrait; margin: 0; }
          *, *::before, *::after { -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
          body * { visibility: hidden !important; }
          .clnav-doc, .clnav-doc * { visibility: visible !important; }
          .clnav-doc { position: absolute !important; top: 0 !important; left: 0 !important; width: 850px !important; min-height: 1202px !important; overflow: hidden !important; zoom: 0.934 !important; box-shadow: none !important; margin: 0 !important; }
          .clnav-zoom { zoom: 1 !important; width: 100% !important; }
          .clnav-ctrl { display: none !important; }
        }
      `}</style>
      <div className="clnav-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
      ...
      <div className="clnav-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
        <div className="clnav-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          {/* Header */}
