"use client";
// ─── Anschreiben Template: Botanical ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#2d6a4f", BG:"#fdfbf7", HD:"#f4f8f0", S2:"#e8f4e0", CT:"#1a2e1a", CB:"#2d4a2d", CM:"#6a8c6a" };
...
export default function CL_Botanical(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clbotan_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clbotan_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clbotan_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clbotan_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8f0e0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(45,106,79,0.15)",overflow:"visible",position:"relative"}}>
