...
const DEFAULT_COLORS = { A:"#fbbf24", BG:"#050505", HD:"#0f0f0f", S2:"#181818", CT:"#f5f5f5", CB:"#d0d0d0", CM:"#808080" };
...
export default function CL_NeoNoir(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clnnoir_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clnnoir_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clnnoir_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clnnoir_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#000000",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 40px rgba(251,191,36,0.15)",overflow:"visible",position:"relative"}}>
