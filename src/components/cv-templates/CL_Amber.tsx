"use client";
// ─── Anschreiben Template: Amber (Warm Whisky, Cream Paper) ─────────────────
...
const DEFAULT_COLORS = { A:"#c47a20", BG:"#fdf8ee", HD:"#f5edda", S2:"#ede4cc", CT:"#2a1c08", CB:"#3a2c14", CM:"#8a7040" };
...
const PFX="clamb";
const DARK="#2a1c08"; const RUST="#a05a18";
...
export default function CL_Amber(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clamb_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clamb_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clamb_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clamb_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8ddc8",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        ...
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${RUST}`,background:"transparent",color:RUST,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(80,50,10,0.12)",overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{background:`linear-gradient(135deg, ${HD} 0%, ${hex2rgba(A,0.1)} 100%)`,padding:"32px 48px 22px",borderBottom:`3px solid ${A}`}}>
            <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:28,fontWeight:800,color:CT,display:"block"}}/>
          ...
          <div style={{padding:"28px 48px 48px"}}>
            ...
            <div style={{marginBottom:16,padding:"8px 14px",background:hex2rgba(A,0.08),borderLeft:`3px solid ${A}`,borderBottom:`1px solid ${hex2rgba(A,0.2)}`}}>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{height:2,width:90,background:A,marginBottom:8,opacity:0.6,borderRadius:1}}/>
