"use client";
// ─── Anschreiben Template: Solarpunk ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#4caf50", BG:"#f5fff0", HD:"#e8f8e0", S2:"#d8f0c8", CT:"#0a2a00", CB:"#1a3a0a", CM:"#5a8a4a" };
...
export default function CL_Solarpunk(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clsplnk_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clsplnk_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clsplnk_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clsplnk_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#d8f0c8",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 30px rgba(76,175,80,0.2)",overflow:"visible",position:"relative"}}>
