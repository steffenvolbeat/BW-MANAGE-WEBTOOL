"use client";
// ─── Anschreiben Template: Scarlet (Bold Red, High Contrast) ────────────────
...
const DEFAULT_COLORS = { A:"#c0152a", BG:"#fff8f8", HD:"#1a0505", S2:"#fde0e0", CT:"#ffffff", CB:"#fde0e0", CM:"#cc8888" };
...
const PFX="clsclt";
const GOLD="#b89a20"; const DARK="#1a0505";
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=4}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.1),border:`1px solid ${A}66`,padding:"2px 5px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
export default function CL_Scarlet(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clsclt_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clsclt_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clsclt_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clsclt_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8d0d0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 6px 30px ${hex2rgba(A,0.2)}`,overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          {/* Scarlet header */}
          <div style={{background:HD,padding:"32px 48px 22px",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",right:-30,top:0,width:120,height:"100%",background:A,transform:"skewX(-6deg)",transformOrigin:"top right"}}/>
          ...
          <div style={{height:4,background:`repeating-linear-gradient(90deg, ${A} 0, ${A} 8px, ${GOLD} 8px, ${GOLD} 10px, ${A} 10px)`}}/>

          <div style={{padding:"28px 48px 48px"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:22}}>
              <div>
                {(["company","street","cityZip"] as const).map(k=><E key={k} value={data.recipient[k]} onChange={v=>setR({[k]:v})} editing={editing} style={{fontSize:11,color:"#3a0c0c",display:"block",lineHeight:1.6}}/>)}
              </div>
              <E value={data.date} onChange={v=>setData(d=>({...d,date:v}))} editing={editing} style={{fontSize:11,color:CM,fontStyle:"italic"}}/>
            </div>
            <div style={{marginBottom:16,padding:"8px 14px",background:hex2rgba(A,0.06),borderLeft:`4px solid ${A}`}}>
              <E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:13,fontWeight:800,color:A}}/>
            </div>
            <E value={data.salutation} onChange={v=>setData(d=>({...d,salutation:v}))} editing={editing} style={{fontSize:11,color:"#3a0c0c",display:"block",marginBottom:14}}/>
            ...
            <div style={{marginTop:20}}>
              <E value={data.closing} onChange={v=>setData(d=>({...d,closing:v}))} editing={editing} style={{fontSize:11,color:"#3a0c0c",display:"block",marginBottom:24}}/>
              <div style={{display:"flex",gap:2,marginBottom:8}}>{Array.from({length:5}).map((_,i)=><div key={i} style={{width:8,height:4,background:i===0?A:hex2rgba(A,0.2+i*0.15),borderRadius:1}}/>)}</div>
              <E value={data.signatureName} onChange={v=>setData(d=>({...d,signatureName:v}))} editing={editing} style={{fontSize:13,fontWeight:800,color:DARK}}/>
            </div>
          </div>
        </div>
      </div>
