"use client";
// ─── Anschreiben Template: Galaxy (Deep Space, Nebula, Dark) ─────────────────
...
const DEFAULT_COLORS = { A:"#c084fc", BG:"#06040f", HD:"#0d0825", S2:"#120e30", CT:"#f0e8ff", CB:"#d8c8ff", CM:"#7060aa" };
...
export default function CL_Galaxy(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clglx_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clglx_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clglx_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clglx_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#030208",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${A}`,background:editing?A:"transparent",color:editing?"#000":A,display:"flex",alignItems:"center",gap:6,textShadow:editing?"none":`0 0 8px ${A}`}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 0 80px ${hex2rgba(A,0.2)}`,overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          <div style={{background:`radial-gradient(ellipse at 30% 60%, ${hex2rgba(PINK,0.25)} 0%, transparent 50%), radial-gradient(ellipse at 80% 30%, ${hex2rgba(CYAN,0.2)} 0%, transparent 50%), ${HD}`,padding:"36px 48px 28px",borderBottom:`1px solid ${A}44`}}>
            <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:28,fontWeight:900,color:CT,display:"block",textShadow:`0 0 20px ${A}`,letterSpacing:"0.03em"}}/>
            <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:11,color:CYAN,display:"block",marginTop:4,letterSpacing:"0.1em",textTransform:"uppercase" as const,textShadow:`0 0 8px ${CYAN}`}}/>
          ...
          <div style={{padding:"28px 48px 48px"}}>
            ...
            <div style={{marginBottom:18,padding:"8px 14px",background:hex2rgba(A,0.08),borderLeft:`3px solid ${A}`,boxShadow:`-4px 0 12px ${hex2rgba(A,0.2)}`}}>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{width:80,height:1,background:`linear-gradient(90deg, ${A}, ${CYAN})`,marginBottom:8}}/>
