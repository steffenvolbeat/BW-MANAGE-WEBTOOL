"use client";
// ─── CV Template: Titanium (Dunkles Metall, Industriell) ─────────────────────
...
const DEFAULT_COLORS = { A:"#90a4ae", BG:"#1c2833", S2:"#212e3a", S3:"#2c3e50", SBG:"#161f28", CT:"#cfd8dc", CB:"#b0bec5", CM:"#607d8b" };
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=3}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}) {
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.08),border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:2,padding:"2px 6px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
function TagList({tags,onChange,editing}:{tags:string[];onChange:(t:string[])=>void;editing:boolean}) {
  ...
  return(<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map((t,i)=>(<div key={i} style={{background:S3,color:CB,border:`1px solid ${hex2rgba(A,0.35)}`,borderRadius:2,padding:"2px 8px",fontSize:10,display:"flex",alignItems:"center",gap:3}}>{t}{editing&&<button type="button" onClick={()=>onChange(tags.filter((_,idx)=>idx!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#ef4444",padding:0,display:"flex"}}><XMarkIcon style={{width:9,height:9}}/></button>}</div>))}{editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:2,padding:"2px 6px",background:"transparent",outline:"none",color:CB,fontFamily:"inherit"}} placeholder="+ Neu"/>}</div>);
}
function SecH({title}:{title:string}) {
  const {A,CT}=useContext(ColCtx);
  return(<div style={{marginBottom:10}}><div style={{display:"flex",alignItems:"center",gap:8,marginBottom:5}}><div style={{width:18,height:2,backgroundColor:A}}/><span style={{fontSize:9,fontWeight:700,letterSpacing:"0.2em",textTransform:"uppercase" as const,color:A}}>{title}</span><div style={{flex:1,height:1,backgroundColor:hex2rgba(A,0.2)}}/></div></div>);
}

export default function CV_Titanium() {
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("titanium",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#0d1520",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        <button onClick={()=>window.print()} style={{padding:"7px 16px",borderRadius:2,fontSize:13,fontWeight:600,cursor:"pointer",border:`1px solid ${A}`,backgroundColor:hex2rgba(A,0.12),color:CT,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",borderRadius:2,fontSize:13,cursor:"pointer",border:`1px solid ${showDesign?A:hex2rgba(A,0.3)}`,backgroundColor:"transparent",color:showDesign?A:CM}}>🎨 Design</button>
        <button onClick={()=>{resetStorage();setData(JSON.parse(JSON.stringify(DEFAULT_CV_DATA)));setPhotoSrc("");setFontKey("fira");setSizeKey("md");setPhotoShapeKey("circle");setClrs(DEFAULT_COLORS);setShowDesign(false);}} style={{padding:"7px 16px",borderRadius:2,fontSize:13,cursor:"pointer",border:`1px solid ${hex2rgba(A,0.3)}`,backgroundColor:"transparent",color:CM,display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(<div style={{width:"100%",background:S2,border:`1px solid ${S3}`,borderRadius:4,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
          <div><div style={{fontSize:9,fontWeight:600,color:CM,marginBottom:7,letterSpacing:"0.2em",textTransform:"uppercase"}}>Schriftart</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{FONTS.map(f=><button key={f.key} onClick={()=>setFontKey(f.key)} style={{padding:"3px 9px",borderRadius:2,border:`1px solid ${fontKey===f.key?A:hex2rgba(A,0.25)}`,background:fontKey===f.key?hex2rgba(A,0.1):"transparent",color:fontKey===f.key?A:CM,fontSize:11,cursor:"pointer",fontFamily:f.family}}>{f.label}</button>)}</div></div>
          <div><div style={{fontSize:9,fontWeight:600,color:CM,marginBottom:7,letterSpacing:"0.2em",textTransform:"uppercase"}}>Schriftgröße</div><div style={{display:"flex",gap:5}}>{FONT_SIZES.map(s=><button key={s.key} onClick={()=>setSizeKey(s.key)} style={{padding:"3px 12px",borderRadius:2,border:`1px solid ${sizeKey===s.key?A:hex2rgba(A,0.25)}`,background:sizeKey===s.key?hex2rgba(A,0.1):"transparent",color:sizeKey===s.key?A:CM,fontSize:11,cursor:"pointer"}}>{s.label}</button>)}</div></div>
          <div><div style={{fontSize:9,fontWeight:600,color:CM,marginBottom:7,letterSpacing:"0.2em",textTransform:"uppercase"}}>Foto-Form</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{PHOTO_SHAPES.map(s=><button key={s.key} onClick={()=>setPhotoShapeKey(s.key)} title={s.label} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"5px 7px",borderRadius:2,border:`1px solid ${photoShapeKey===s.key?A:hex2rgba(A,0.25)}`,background:photoShapeKey===s.key?hex2rgba(A,0.08):"transparent",cursor:"pointer"}}><div style={{width:20,height:s.key==="ellipse"?26:20,borderRadius:s.br,clipPath:s.clip??"",backgroundColor:photoShapeKey===s.key?A:S3}}/><span style={{fontSize:8,color:photoShapeKey===s.key?A:CM,whiteSpace:"nowrap"}}>{s.label}</span></button>)}</div></div>
          <div><div style={{fontSize:9,fontWeight:600,color:CM,marginBottom:7,letterSpacing:"0.2em",textTransform:"uppercase"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Akzent"},{k:"BG" as const,l:"BG"},{k:"S2" as const,l:"Panel"},{k:"S3" as const,l:"Karte"},{k:"SBG" as const,l:"Sidebar"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Meta"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,borderRadius:2,border:`1px solid ${hex2rgba(A,0.3)}`,cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:CM}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",borderRadius:2,fontSize:11,border:`1px solid ${hex2rgba(A,0.3)}`,background:"transparent",color:CM,cursor:"pointer",alignSelf:"center"}}>↺ Reset</button></div></div>
        </div>)}
      </div>
      ...
      <div className={`${PFX}-doc`} style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:"0 2px 20px rgba(0,0,0,0.5)",fontFamily:fnt,overflow:"hidden",border:`1px solid ${S3}`}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          {/* Header */}
          <div style={{backgroundColor:S2,padding:"28px 40px 22px",borderBottom:`1px solid ${S3}`}}>
            <div style={{display:"flex",gap:24,alignItems:"center"}}>
              <div style={{width:curShape.w,height:curShape.h,borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",flexShrink:0,backgroundColor:S3,border:`2px solid ${hex2rgba(A,0.5)}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:36,fontWeight:700,color:CT,lineHeight:1,marginBottom:4,fontFamily:fnt,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.25)}`,borderRadius:2,padding:"2px 8px",outline:"none",width:"100%",boxSizing:"border-box",letterSpacing:"-0.01em"}}/>:<div style={{fontSize:36,fontWeight:700,color:CT,lineHeight:1,marginBottom:5,letterSpacing:"-0.01em",textShadow:`0 0 20px ${hex2rgba(A,0.3)}`}}>{data.personal.name}</div>}
                <div style={{height:1,background:`linear-gradient(90deg,${A},${hex2rgba(A,0.1)})`,marginBottom:6}}/>
                {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:12,color:CM,fontWeight:500,letterSpacing:"0.1em",fontFamily:fnt,background:hex2rgba(A,0.04),border:`1px solid ${hex2rgba(A,0.2)}`,borderRadius:2,padding:"1px 6px",outline:"none",width:"100%",boxSizing:"border-box",marginBottom:10,textTransform:"uppercase"}}/>:<div style={{fontSize:12,color:CM,fontWeight:500,letterSpacing:"0.1em",marginBottom:10,textTransform:"uppercase"}}>{data.personal.subtitle}</div>}
                <E value={data.personal.bio} onChange={v=>setP({bio:v})} editing={editing} multiline rows={2} style={{fontSize:11,color:CB,lineHeight:1.8}}/>
                <div style={{display:"flex",flexWrap:"wrap",gap:14,marginTop:10}}>
                  {(["email","phone","location","github","linkedin"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CVData["personal"]>)} style={{fontSize:10,color:CB,background:hex2rgba(A,0.05),border:`1px solid ${hex2rgba(A,0.2)}`,padding:"2px 8px",outline:"none",fontFamily:"inherit",borderRadius:2}}/>:<span key={k} style={{fontSize:10,color:CM}}>{data.personal[k]}</span>:null)}
                </div>
              </div>
            </div>
          </div>
          <div style={{display:"flex"}}>
            <div style={{flex:1,padding:"22px 22px 48px 40px",minWidth:0}}>
