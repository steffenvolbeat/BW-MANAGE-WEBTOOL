"use client";
// ─── Anschreiben Template: Bamboo (Natural Zen, Green, Minimal) ──────────────
...
const DEFAULT_COLORS = { A:"#5a8a4a", BG:"#f8faf2", HD:"#eef4e8", S2:"#e4eede", CT:"#1e2e18", CB:"#2e3e28", CM:"#728a68" };
...
const PFX="clbam";
const GOLD="#b8982a"; const SAND="#e8d89a";
...
export default function CL_Bamboo(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clbam_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clbam_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clbam_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clbam_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#dce8d0",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 16px rgba(40,80,20,0.08)",overflow:"visible",display:"flex"}}>
        {/* Bamboo stalk decoration left */}
        <div style={{width:18,background:`repeating-linear-gradient(180deg, ${A} 0px, ${A} 40px, ${hex2rgba(A,0.6)} 40px, ${hex2rgba(A,0.6)} 44px)`,flexShrink:0}}/>
        <div style={{flex:1}}>
          <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
            <div style={{background:HD,padding:"30px 44px 20px",borderBottom:`1px solid ${hex2rgba(A,0.3)}`}}>
            ...
            <div style={{padding:"24px 44px 44px"}}>
              ...
              <div style={{marginBottom:14,display:"flex",gap:8,alignItems:"center"}}>
                <span style={{color:GOLD,fontSize:14}}>竹</span>
              ...
              <div style={{marginTop:20}}>
                ...
                <div style={{height:1,width:70,background:A,marginBottom:8,opacity:0.4}}/>
