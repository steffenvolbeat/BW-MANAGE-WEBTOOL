...
const DEFAULT_COLORS = { A:"#e06c9e", BG:"#fffaf5", HD:"#fff0f5", S2:"#ffe8f0", CT:"#3d1a2a", CB:"#5a2a3a", CM:"#c08898" };
...
export default function CL_Washi(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clwashi_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clwashi_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clwashi_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clwashi_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f5e8ee",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(224,108,158,0.15)",overflow:"visible",position:"relative"}}>
