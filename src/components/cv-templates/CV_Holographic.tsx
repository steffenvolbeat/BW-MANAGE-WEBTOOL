...
const DEFAULT_COLORS = { A:"#a855f7", BG:"#06020f", S2:"#0e0520", S3:"#160830", SBG:"#0a0318", CT:"#f0e8ff", CB:"#d0c0f0", CM:"#8060b0" };
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=3}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.1),border:`1px dashed ${A}66`,padding:"2px 4px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
function BulletList({bullets,onChange,editing}:{bullets:string[];onChange:(b:string[])=>void;editing:boolean}){
  ...
  return(<ul style={{listStyle:"none",margin:0,padding:0}}>{bullets.map((b,i)=>(
    <li key={i} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
      <span style={{width:6,height:6,background:"linear-gradient(135deg,#f0f,#0ff,#ff0)",flexShrink:0,marginTop:4,borderRadius:"50%",display:"inline-block"}}/>
...
function TagList({tags,onChange,editing}:{tags:string[];onChange:(t:string[])=>void;editing:boolean}){
  ...
  return(<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map((t,i)=>(<div key={i} style={{background:"linear-gradient(135deg,rgba(168,85,247,0.2),rgba(0,255,255,0.1))",border:`1px solid transparent`,backgroundClip:"padding-box",color:CT,padding:"2px 10px",fontSize:10,display:"flex",alignItems:"center",gap:3,borderRadius:4,boxShadow:`0 0 8px ${hex2rgba(A,0.3)}`}}>{t}{editing&&<button type="button" onClick={()=>onChange(tags.filter((_,j)=>j!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#f87171",padding:0}}><XMarkIcon style={{width:9,height:9}}/></button>}</div>))}
  {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px dashed ${A}`,borderRadius:4,padding:"2px 5px",background:hex2rgba(A,0.1),outline:"none",color:CT,fontFamily:"inherit"}} placeholder="+ Neu"/>}</div>);
}
function SecH({title}:{title:string}){
  ...
  return(<div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
    <div style={{width:14,height:14,background:"linear-gradient(135deg,#f0f,#0ff,#ff0,#f0f)",borderRadius:"50%",flexShrink:0}}/>
    <span style={{fontSize:11,fontWeight:800,letterSpacing:"0.12em",textTransform:"uppercase" as const,background:"linear-gradient(90deg,#f0f,#0ff,#a855f7)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{title}</span>
    <div style={{flex:1,height:1,background:"linear-gradient(90deg,#f0f55,#0ff55,transparent)"}}/>
  </div>);
}

export default function CV_Holographic(){
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("holographic",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#000",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
        ${curFont.gf?`@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');`:""}.${PFX}-doc,.${PFX}-doc *{font-family:${fnt}!important;}
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${showDesign?A:"#444"}`,backgroundColor:"transparent",color:showDesign?A:"#888"}}>🎨 Design</button>
        <button onClick={()=>{resetStorage();setData(JSON.parse(JSON.stringify(DEFAULT_CV_DATA)));setPhotoSrc("");setFontKey("nunito");setSizeKey("md");setPhotoShapeKey("square");setClrs(DEFAULT_COLORS);setShowDesign(false);}} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:"2px solid #444",backgroundColor:"transparent",color:"#888",display:"flex",alignItems:"center",gap:6}}><XMarkIcon style={{width:16,height:16}}/>Reset</button>
        {showDesign&&(<div style={{width:"100%",background:"#111",border:"1px solid #333",padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
          <div><div style={{fontSize:10,fontWeight:700,color:"#666",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Font</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{FONTS.map(f=><button key={f.key} onClick={()=>setFontKey(f.key)} style={{padding:"3px 9px",border:`1px solid ${fontKey===f.key?A:"#444"}`,background:fontKey===f.key?hex2rgba(A,0.15):"transparent",color:fontKey===f.key?A:"#666",fontSize:11,cursor:"pointer",fontFamily:f.family}}>{f.label}</button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:"#666",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Größe</div><div style={{display:"flex",gap:5}}>{FONT_SIZES.map(s=><button key={s.key} onClick={()=>setSizeKey(s.key)} style={{padding:"3px 12px",border:`1px solid ${sizeKey===s.key?A:"#444"}`,background:sizeKey===s.key?hex2rgba(A,0.15):"transparent",color:sizeKey===s.key?A:"#666",fontSize:11,cursor:"pointer"}}>{s.label}</button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:"#666",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Foto</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{PHOTO_SHAPES.map(s=><button key={s.key} onClick={()=>setPhotoShapeKey(s.key)} style={{padding:"4px 8px",border:`2px solid ${photoShapeKey===s.key?A:"#444"}`,background:photoShapeKey===s.key?hex2rgba(A,0.15):"transparent",cursor:"pointer",fontSize:10,color:photoShapeKey===s.key?A:"#666"}}>{s.label}</button>)}</div></div>
          <div><div style={{fontSize:10,fontWeight:700,color:"#666",marginBottom:7,textTransform:"uppercase",letterSpacing:"0.1em"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Akzent"},{k:"BG" as const,l:"BG"},{k:"S2" as const,l:"Panel"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Gedimmt"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,border:"1px solid #444",cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:"#666"}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",fontSize:11,border:"1px solid #444",background:"transparent",color:"#666",cursor:"pointer",alignSelf:"center"}}>↺</button></div></div>
        </div>)}
      </div>

      <div ref={docRef} className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,overflow:"visible",fontFamily:fnt,position:"relative",boxShadow:"0 0 80px rgba(168,85,247,0.4),0 0 160px rgba(0,255,255,0.1)"}}>
        {/* Holographic rainbow border */}
        <div style={{position:"absolute",inset:0,border:"2px solid transparent",background:"linear-gradient(#06020f,#06020f) padding-box,linear-gradient(135deg,#f0f,#0ff,#ff0,#0f0,#f0f) border-box",pointerEvents:"none"}}/>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale,position:"relative"}}>
          <div style={{background:"linear-gradient(135deg,#0e0520,#1a0840,#0e0520)",padding:"32px 40px 24px",borderBottom:"2px solid transparent",backgroundClip:"padding-box",position:"relative",overflow:"hidden"}}>
            <div style={{position:"absolute",inset:0,background:"linear-gradient(135deg,rgba(255,0,255,0.05),rgba(0,255,255,0.05),rgba(255,255,0,0.03))",pointerEvents:"none"}}/>
            <div style={{display:"flex",gap:28,alignItems:"center",position:"relative"}}>
              <div style={{position:"relative",width:curShape.w,height:curShape.h,flexShrink:0,cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
                <div style={{width:"100%",height:"100%",borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",backgroundColor:S3,border:"2px solid transparent",backgroundClip:"padding-box",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 0 30px rgba(168,85,247,0.6),0 0 60px rgba(0,255,255,0.2)"}}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:32,fontWeight:900,color:CT,marginBottom:4,fontFamily:"inherit",background:hex2rgba(A,0.1),border:`1px dashed ${A}`,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:32,fontWeight:900,marginBottom:4,background:"linear-gradient(90deg,#f0f,#a855f7,#0ff)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{data.personal.name}</div>}
                {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:13,color:A,marginBottom:8,fontFamily:"inherit",background:hex2rgba(A,0.1),border:`1px dashed ${A}`,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:13,fontWeight:700,color:A,marginBottom:8,letterSpacing:"0.1em"}}>{data.personal.subtitle}</div>}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:10}}>
                  {(["email","phone","location","github","linkedin","website"] as const).map(k=>editing
                    ?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CVData["personal"]>)} style={{fontSize:10,color:CT,background:hex2rgba(A,0.1),border:`1px dashed ${A}44`,padding:"2px 8px",outline:"none",fontFamily:"inherit"}}/>
                    :data.personal[k]?<span key={k} style={{fontSize:10,color:A,background:hex2rgba(A,0.12),padding:"2px 10px",border:`1px solid ${A}44`,borderRadius:2}}>{data.personal[k]}</span>:null)}
                </div>
              </div>
            </div>
          </div>
            ...
            <div style={{width:220,background:S2,padding:"22px 18px",flexShrink:0,borderRight:`1px solid ${A}22`}}>
              ...
              <div style={{marginBottom:20}}><SecH title="Sprachen"/>
                {data.languages.map((l,i)=>(<div key={i} style={{marginBottom:8}}>
                  ...
                  <div style={{height:3,background:`${A}22`}}><div style={{height:"100%",width:l.level==="Muttersprache"?"100%":l.level==="Verhandlungssicher"?"85%":l.level==="Fließend"?"70%":l.level==="Grundkenntnisse"?"40%":"60%",background:"linear-gradient(90deg,#f0f,#0ff)",boxShadow:`0 0 6px ${A}`}}/></div>
                </div>))}
              </div>
              <div style={{marginBottom:20}}><SecH title="Ausbildung"/>
                {data.education.map(e=>(<div key={e.id} style={{marginBottom:12,paddingLeft:8,borderLeft:`2px solid ${A}66`}}>
            ...
            <div style={{flex:1,padding:"22px 28px"}}>
              <div style={{marginBottom:20}}><SecH title="Berufserfahrung"/>
                {data.experience.map(ex=>(<div key={ex.id} style={{marginBottom:16,paddingLeft:12,borderLeft:`2px solid ${A}55`,position:"relative"}}>
                  <div style={{position:"absolute",left:-5,top:4,width:8,height:8,borderRadius:"50%",background:"linear-gradient(135deg,#f0f,#0ff)",boxShadow:`0 0 8px ${A}`}}/>
              ...
              <div style={{marginBottom:20}}><SecH title="Projekte"/>
                {data.projects.map(p=>(<div key={p.id} style={{marginBottom:14,paddingLeft:12,borderLeft:`2px solid ${A}33`,position:"relative"}}>
                  <div style={{position:"absolute",left:-4,top:4,width:6,height:6,borderRadius:"50%",background:A}}/>
