"use client";
// ─── Anschreiben Template: Engraving ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#8b4513", BG:"#fdf8f0", HD:"#f5edd8", S2:"#ece0c4", CT:"#2a1a0a", CB:"#3d2510", CM:"#8b6a40" };
...
export default function CL_Engraving(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clengr_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clengr_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clengr_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clengr_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8d8c0",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(139,69,19,0.2)",overflow:"visible",position:"relative"}}>
