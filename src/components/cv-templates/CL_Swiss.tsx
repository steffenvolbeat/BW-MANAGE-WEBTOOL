"use client";
// ─── Anschreiben Template: Swiss (International Typographic Style) ────────────
...
const DEFAULT_COLORS = { A:"#e63012", BG:"#ffffff", HD:"#f0f0f0", S2:"#f8f8f8", CT:"#000000", CB:"#1a1a1a", CM:"#666666" };
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=4}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.04),border:`1px dashed ${A}66`,padding:"2px 4px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
export default function CL_Swiss(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clswi_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clswi_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clswi_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clswi_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8e8e8",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        ...
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:"2px solid #333",background:"transparent",color:"#333",display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 2px 12px rgba(0,0,0,0.12)",overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          {/* Swiss style top red bar + grid header */}
          <div style={{display:"flex",height:80}}>
            <div style={{width:14,background:A}}/>
            <div style={{flex:1,background:CT,display:"flex",alignItems:"center",padding:"0 44px"}}>
              <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:26,fontWeight:900,color:BG,letterSpacing:"-0.01em"}}/>
            </div>
            <div style={{width:200,background:A,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:9,color:"white",textTransform:"uppercase" as const,letterSpacing:"0.1em",textAlign:"center" as const}}/>
            </div>
          </div>
          {/* Contact strip */}
          ...
          <div style={{padding:"32px 48px 48px"}}>
            ...
            <div style={{marginBottom:20,display:"flex",gap:14,alignItems:"center"}}>
              <div style={{width:4,height:20,background:A,flexShrink:0}}/>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{display:"flex",gap:10,alignItems:"center",marginBottom:8}}><div style={{width:40,height:2,background:A}}/><div style={{width:4,height:2,background:CM}}/></div>
