"use client";
// ─── Anschreiben Template: Crimson (Rot/Dunkelrot) ────────────────────────────
...
const A = "#ef4444"; const DARK = "#7f1d1d";
...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(255,255,255,0.1)" : "rgba(239,68,68,0.05)", border: `1px dashed ${inv ? "rgba(255,255,255,0.4)" : `${A}55`}`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Crimson() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#450a0a", padding: "24px 16px" }}>
      <style>{`${curFont.gf ? `@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');` : ""} .clcri-doc,.clcri-doc * { font-family: ${fnt} !important; } @media print { @page{size:A4 portrait;margin:0} body *{visibility:hidden!important} .clcri-doc,.clcri-doc *{visibility:visible!important} .clcri-doc{position:absolute!important;top:0!important;left:0!important;width:210mm!important;box-shadow:none!important;margin:0!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important} .clcri-zoom{zoom:1!important;width:100%!important} .clcri-ctrl{display:none!important} }`}</style>
      <div className="clcri-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {showDesign && <div style={{ width: "100%", background: "#450a0a", border: `1px solid ${A}44`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#b91c1c", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#b91c1c"}`, background: fontKey === f.key ? "rgba(239,68,68,0.1)" : "transparent", color: fontKey === f.key ? A : "#6b7280", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#b91c1c", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Größe</div><div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#b91c1c"}`, background: sizeKey === s.key ? "rgba(239,68,68,0.1)" : "transparent", color: sizeKey === s.key ? A : "#6b7280", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div></div>
        </div>}
      </div>
      <div className="clcri-doc" style={{ width: 850, margin: "0 auto", backgroundColor: "white", boxShadow: `0 8px 50px rgba(239,68,68,0.2)`, overflow: "hidden" }}>
        <div className="clcri-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          <div style={{ background: A, padding: "28px 48px 22px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end" }}>
              <div><E value={data.personal.name} onChange={v => setP({ name: v })} editing={editing} inv style={{ fontSize: 30, fontWeight: 900, color: "white", display: "block", marginBottom: 3 }} /><E value={data.personal.subtitle} onChange={v => setP({ subtitle: v })} editing={editing} inv style={{ fontSize: 11, color: "rgba(255,255,255,0.8)", display: "block" }} /></div>
          ...
          <div style={{ height: 4, background: DARK }} />
          <div style={{ padding: "32px 48px 48px", color: "#374151" }}>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ width: 80, height: 3, backgroundColor: A, borderRadius: 2, marginBottom: 8 }} />
