...
const DEFAULT_COLORS = { A:"#ff6b35", BG:"#0a0015", HD:"#120020", S2:"#1a0030", CT:"#ffe8d8", CB:"#f0c0a0", CM:"#a06040" };
...
export default function CL_Lava(){
  const [data,setData]=usePersistentCLState<CLData>('cl_cllava_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_cllava_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_cllava_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_cllava_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#050008",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 60px rgba(255,107,53,0.3)",overflow:"visible",position:"relative"}}>
