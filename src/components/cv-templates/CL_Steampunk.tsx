"use client";
// ─── Anschreiben Template: Steampunk (Victorian, Brass, Gears) ───────────────
...
const DEFAULT_COLORS = { A:"#b87333", BG:"#1a1208", HD:"#261a0a", S2:"#2e2010", CT:"#f0dda0", CB:"#d4c080", CM:"#8a7240" };
...
const PFX="clstmp";
const GOLD="#d4a840"; const RUST="#8b4513";
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=4}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.08),border:`1px solid ${A}55`,padding:"2px 5px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
export default function CL_Steampunk(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clstmp_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clstmp_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clstmp_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clstmp_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#110c05",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${A}`,background:editing?A:"transparent",color:editing?"#1a1208":A,display:"flex",alignItems:"center",gap:6}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:`0 0 40px ${hex2rgba(A,0.2)}`,overflow:"visible",border:`2px solid ${A}44`}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          {/* Riveted corner decorations */}
          <div style={{background:HD,padding:"32px 48px 20px",borderBottom:`3px solid ${A}`,position:"relative"}}>
            <div style={{position:"absolute",top:8,left:8,width:10,height:10,borderRadius:"50%",background:A,boxShadow:`0 0 4px ${GOLD}`}}/>
            <div style={{position:"absolute",top:8,right:8,width:10,height:10,borderRadius:"50%",background:A,boxShadow:`0 0 4px ${GOLD}`}}/>
            <div style={{fontSize:9,color:A,letterSpacing:"0.15em",textTransform:"uppercase" as const,marginBottom:8,fontStyle:"italic"}}>— Bewerbungsschreiben —</div>
            <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:26,fontWeight:900,color:CT,display:"block",letterSpacing:"0.04em"}}/>
            <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:11,color:GOLD,display:"block",marginTop:4,fontStyle:"italic"}}/>
          ...
          <div style={{padding:"28px 48px 48px"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:22}}>
              <div style={{padding:"10px 14px",border:`1px solid ${A}44`,background:hex2rgba(A,0.06)}}>
            ...
            <div style={{marginBottom:16,display:"flex",gap:8,alignItems:"center",padding:"6px 12px",background:hex2rgba(A,0.1),border:`1px solid ${A}44`}}>
              <span style={{color:A,fontSize:14}}>⚙</span>
              <E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:12,fontWeight:700,color:GOLD}}/>
            </div>
            ...
            <div style={{marginTop:20}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:8}}>
                <div style={{flex:1,height:1,background:`repeating-linear-gradient(90deg, ${A}66 0, ${A}66 4px, transparent 4px, transparent 8px)`}}/>
