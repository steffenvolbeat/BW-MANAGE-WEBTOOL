...
const DEFAULT_COLORS = { A:"#c9a227", BG:"#fafaf5", HD:"#f5f0e0", S2:"#ede8d0", CT:"#1a1200", CB:"#2a1e00", CM:"#806040" };
...
export default function CL_Cairo(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clcairo_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clcairo_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clcairo_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clcairo_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8e0c8",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(201,162,39,0.15)",overflow:"visible",position:"relative"}}>
