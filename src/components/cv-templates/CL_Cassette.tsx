...
const DEFAULT_COLORS = { A:"#f0e000", BG:"#0a0a0a", HD:"#141414", S2:"#1e1e1e", CT:"#ffffff", CB:"#e0e0e0", CM:"#888888" };
...
export default function CL_Cassette(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clcass_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clcass_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clcass_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clcass_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#050505",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 30px rgba(240,224,0,0.2)",overflow:"visible",position:"relative"}}>
