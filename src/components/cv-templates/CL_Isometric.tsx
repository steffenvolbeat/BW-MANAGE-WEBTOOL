"use client";
// ─── Anschreiben Template: Isometric ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#00c8ff", BG:"#0a1628", HD:"#0f1f3d", S2:"#1a2840", CT:"#e0f8ff", CB:"#a0d8ee", CM:"#4080a0" };
...
export default function CL_Isometric(){
  const [data,setData]=usePersistentCLState<CLData>('cl_cliso_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_cliso_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_cliso_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_cliso_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#050e1e",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 40px rgba(0,200,255,0.2)",overflow:"visible",position:"relative"}}>
