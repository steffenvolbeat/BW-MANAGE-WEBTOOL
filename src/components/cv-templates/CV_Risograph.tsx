...
const DEFAULT_COLORS = { A:"#e8416e", BG:"#fdf4e3", S2:"#fff0d8", S3:"#fde8c8", SBG:"#fff8f0", CT:"#1a0a10", CB:"#3a1520", CM:"#a06878" };
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=3}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.07),border:`1px dashed ${A}66`,padding:"2px 4px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
function BulletList({bullets,onChange,editing}:{bullets:string[];onChange:(b:string[])=>void;editing:boolean}){
  ...
  return(<ul style={{listStyle:"none",margin:0,padding:0}}>{bullets.map((b,i)=>(
    <li key={i} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
      <span style={{width:8,height:8,borderRadius:"50%",background:A,flexShrink:0,marginTop:4,opacity:0.85}}/>
...
function TagList({tags,onChange,editing}:{tags:string[];onChange:(t:string[])=>void;editing:boolean}){
  ...
  return(<div style={{display:"flex",flexWrap:"wrap",gap:4}}>{tags.map((t,i)=>(<div key={i} style={{background:hex2rgba(A,0.12),border:`1.5px solid ${A}88`,color:CT,padding:"2px 8px",fontSize:10,display:"flex",alignItems:"center",gap:3,borderRadius:"999px"}}>{t}{editing&&<button type="button" onClick={()=>onChange(tags.filter((_,j)=>j!==i))} style={{background:"none",border:"none",cursor:"pointer",color:"#f87171",padding:0}}><XMarkIcon style={{width:9,height:9}}/></button>}</div>))}
  {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px dashed ${A}`,borderRadius:"999px",padding:"2px 8px",background:hex2rgba(A,0.05),outline:"none",color:CT,fontFamily:"inherit"}} placeholder="+ Neu"/>}</div>);
}
function SecH({title}:{title:string}){
  const {A,CT}=useContext(ColCtx);
  return(<div style={{marginBottom:10,position:"relative"}}>
    <div style={{position:"absolute",left:-4,top:-2,width:"calc(100% + 8px)",height:"110%",background:hex2rgba(A,0.15),transform:"rotate(-0.5deg)"}}/>
    <span style={{fontSize:11,fontWeight:800,letterSpacing:"0.1em",textTransform:"uppercase" as const,color:CT,position:"relative"}}>{title}</span>
  </div>);
}

export default function CV_Risograph(){
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("risograph",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f0e8d8",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        {showDesign&&(<div style={{width:"100%",background:"#fff",border:"1px solid #ddd",padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
      ...
      <div ref={docRef} className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,overflow:"visible",fontFamily:fnt,position:"relative",boxShadow:"6px 6px 0 rgba(232,65,110,0.3),12px 12px 0 rgba(0,160,180,0.15)"}}>
        {/* Risograph halftone dots overlay */}
        <svg style={{position:"absolute",top:0,left:0,width:"100%",height:"100%",opacity:0.04,pointerEvents:"none"}} xmlns="http://www.w3.org/2000/svg">
          <defs><pattern id="riso-dots" width="6" height="6" patternUnits="userSpaceOnUse"><circle cx="3" cy="3" r="1" fill={A}/></pattern></defs>
          <rect width="100%" height="100%" fill="url(#riso-dots)"/>
        </svg>
        {/* Color offset shadow top bar */}
        <div style={{position:"absolute",top:4,left:4,width:"100%",height:80,background:"#00c8c8",opacity:0.15,pointerEvents:"none"}}/>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale,position:"relative"}}>
          {/* Header */}
          <div style={{background:S2,padding:"32px 40px 24px",borderBottom:`4px solid ${A}`,position:"relative"}}>
            <div style={{position:"absolute",top:0,right:0,width:200,height:"100%",background:hex2rgba(A,0.06),clipPath:"polygon(40px 0%,100% 0%,100% 100%,0% 100%)"}}/>
            <div style={{display:"flex",gap:28,alignItems:"center"}}>
              <div style={{position:"relative",width:curShape.w,height:curShape.h,flexShrink:0,cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
                <div style={{width:"100%",height:"100%",borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",backgroundColor:S3,border:`3px solid ${A}`,display:"flex",alignItems:"center",justifyContent:"center",boxShadow:`4px 4px 0 #00c8c8`}}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:32,fontWeight:900,color:CT,marginBottom:4,fontFamily:"inherit",background:hex2rgba(A,0.08),border:`1px dashed ${A}`,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:32,fontWeight:900,color:CT,marginBottom:4}}>{data.personal.name}</div>}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:10}}>
                  {(["email","phone","location","github","linkedin","website"] as const).map(k=>editing
                    ...
                    :data.personal[k]?<span key={k} style={{fontSize:10,color:CM,background:S3,padding:"2px 10px",border:`1px solid ${A}44`,borderRadius:2}}>{data.personal[k]}</span>:null)}
                </div>
              </div>
            </div>
          </div>
          <div style={{display:"flex"}}>
            <div style={{width:220,background:S2,padding:"22px 18px",flexShrink:0,borderRight:`3px solid ${A}22`}}>
              ...
              <div style={{marginBottom:20}}><SecH title="Sprachen"/>
                {data.languages.map((l,i)=>(<div key={i} style={{marginBottom:8}}>
                  ...
                  <div style={{height:4,background:`${A}18`,borderRadius:2}}><div style={{height:"100%",width:l.level==="Muttersprache"?"100%":l.level==="Verhandlungssicher"?"85%":l.level==="Fließend"?"70%":l.level==="Grundkenntnisse"?"40%":"60%",background:A,borderRadius:2}}/></div>
                </div>))}
              </div>
            ...
            <div style={{flex:1,padding:"22px 28px"}}>
              <div style={{marginBottom:20}}><SecH title="Berufserfahrung"/>
                {data.experience.map(ex=>(<div key={ex.id} style={{marginBottom:16,paddingLeft:12,borderLeft:`3px solid ${A}`}}>
              ...
              <div style={{marginBottom:20}}><SecH title="Projekte"/>
                {data.projects.map(p=>(<div key={p.id} style={{marginBottom:14,paddingLeft:12,borderLeft:`3px solid ${A}55`}}>
