"use client";
// ─── Anschreiben Template: Holographic ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#a855f7", BG:"#06020f", HD:"#0e0520", S2:"#160830", CT:"#f0e8ff", CB:"#d0c0f0", CM:"#8060b0" };
...
export default function CL_Holographic(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clholo_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clholo_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clholo_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clholo_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#020008",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 0 60px rgba(168,85,247,0.3)",overflow:"visible",position:"relative"}}>
