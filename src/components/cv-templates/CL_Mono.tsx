      ...
      <div className="clmno-doc" style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:"0 2px 20px rgba(0,0,0,0.15)",fontFamily:fnt,overflow:"hidden"}}>
        <div className="clmno-zoom" style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          <div style={{backgroundColor:HD,padding:"44px 64px 28px",borderBottom:`1px solid #e0e0e0`}}>
            ...
            <div style={{display:"flex",gap:28,flexWrap:"wrap",marginTop:14}}>{(["email","phone","location","website"] as const).map(k=>data.personal[k]||editing?editing?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CLData["personal"]>)} style={{fontSize:10,color:CM,background:"#f4f4f4",border:"1px solid #ddd",padding:"2px 8px",outline:"none",fontFamily:"inherit",borderRadius:0}}/>:<span key={k} style={{fontSize:10,color:CM}}>{data.personal[k]}</span>:null)}</div>
          </div>
