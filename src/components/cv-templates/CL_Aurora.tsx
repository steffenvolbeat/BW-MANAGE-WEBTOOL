"use client";
// ─── Anschreiben Template: Aurora (Purple/Pink Gradient) ─────────────────────
...
const A = "#a855f7"; const A2 = "#ec4899";
...
function E({ value, onChange, editing, multiline = false, style = {} as React.CSSProperties, placeholder = "...", rows = 4, inv = false }: {
  ...
  const base: React.CSSProperties = { ...style, background: inv ? "rgba(255,255,255,0.1)" : "rgba(168,85,247,0.06)", border: `1px dashed ${inv ? "rgba(255,255,255,0.4)" : `${A}55`}`, borderRadius: 3, padding: "2px 4px", outline: "none", width: "100%", fontFamily: "inherit", fontSize: "inherit", color: "inherit", lineHeight: "inherit", fontWeight: "inherit", boxSizing: "border-box" };
...
export default function CL_Aurora() {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1e1b4b", padding: "24px 16px" }}>
      <style>{`${curFont.gf ? `@import url('https://fonts.googleapis.com/css2?family=${curFont.gf}&display=swap');` : ""} .claur-doc,.claur-doc * { font-family: ${fnt} !important; } @media print { @page{size:A4 portrait;margin:0} body *{visibility:hidden!important} .claur-doc,.claur-doc *{visibility:visible!important} .claur-doc{position:absolute!important;top:0!important;left:0!important;width:210mm!important;box-shadow:none!important;margin:0!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important} .claur-zoom{zoom:1!important;width:100%!important} .claur-ctrl{display:none!important} }`}</style>
      <div className="claur-ctrl" style={{ maxWidth: 850, margin: "0 auto 16px", display: "flex", gap: 8, flexWrap: "wrap" }}>
        ...
        {showDesign && <div style={{ width: "100%", background: "#2e1065", border: `1px solid ${A}44`, borderRadius: 10, padding: "14px 18px", display: "flex", gap: 24, flexWrap: "wrap" }}>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#7c3aed", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Schriftart</div><div style={{ display: "flex", gap: 5, flexWrap: "wrap" }}>{FONTS.map(f => <button key={f.key} onClick={() => setFontKey(f.key)} style={{ padding: "3px 9px", borderRadius: 5, border: `1px solid ${fontKey === f.key ? A : "#374151"}`, background: fontKey === f.key ? "rgba(168,85,247,0.12)" : "transparent", color: fontKey === f.key ? A : "#888", fontSize: 11, cursor: "pointer", fontFamily: f.family }}>{f.label}</button>)}</div></div>
          <div><div style={{ fontSize: 10, fontWeight: 700, color: "#7c3aed", marginBottom: 7, textTransform: "uppercase", letterSpacing: "0.1em" }}>Größe</div><div style={{ display: "flex", gap: 5 }}>{FONT_SIZES.map(s => <button key={s.key} onClick={() => setSizeKey(s.key)} style={{ padding: "3px 12px", borderRadius: 5, border: `1px solid ${sizeKey === s.key ? A : "#374151"}`, background: sizeKey === s.key ? "rgba(168,85,247,0.12)" : "transparent", color: sizeKey === s.key ? A : "#888", fontSize: 11, cursor: "pointer" }}>{s.label}</button>)}</div></div>
        </div>}
      </div>
      <div className="claur-doc" style={{ width: 850, margin: "0 auto", backgroundColor: "white", boxShadow: `0 0 60px rgba(168,85,247,0.3)`, overflow: "hidden" }}>
        <div className="claur-zoom" style={{ width: Math.round(850 / scale), zoom: scale }}>
          <div style={{ background: `linear-gradient(135deg,#1e1b4b,#6d28d9,${A2})`, padding: "30px 48px 24px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
              <div><E value={data.personal.name} onChange={v => setP({ name: v })} editing={editing} inv style={{ fontSize: 28, fontWeight: 900, color: "white", display: "block", marginBottom: 3 }} /><E value={data.personal.subtitle} onChange={v => setP({ subtitle: v })} editing={editing} inv style={{ fontSize: 11, color: "rgba(255,255,255,0.75)", display: "block" }} /></div>
              <div style={{ textAlign: "right" }}>{[data.personal.email, data.personal.phone, data.personal.location].map((v, i) => <E key={i} value={v} onChange={nv => setP([{ email: nv }, { phone: nv }, { location: nv }][i])} editing={editing} inv style={{ fontSize: 10, color: "rgba(255,255,255,0.7)", display: "block", marginBottom: 2 }} />)}</div>
            </div>
          </div>
          <div style={{ padding: "32px 48px 48px", color: "#374151" }}>
            ...
            <div style={{ marginTop: 20 }}>
              ...
              <div style={{ width: 100, height: 2, background: `linear-gradient(90deg,${A},${A2})`, borderRadius: 1, marginBottom: 8 }} />
