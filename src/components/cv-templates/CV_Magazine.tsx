...
const DEFAULT_COLORS = { A:"#dc2626", BG:"#ffffff", S2:"#f5f5f5", S3:"#e8e8e8", SBG:"#f8f8f8", CT:"#111111", CB:"#222222", CM:"#555555" };
...
function E({ value, onChange, editing, multiline=false, style={} as React.CSSProperties, placeholder="...", rows=3 }:{ value:string; onChange:(v:string)=>void; editing:boolean; multiline?:boolean; style?:React.CSSProperties; placeholder?:string; rows?:number; }) {
  ...
  const s: React.CSSProperties = { ...style, background: hex2rgba(A,0.06), border:`1px solid ${hex2rgba(A,0.35)}`, borderRadius:2, padding:"2px 4px", outline:"none", width:"100%", fontFamily:"inherit", fontSize:"inherit", color:"inherit", lineHeight:"inherit", fontWeight:"inherit", boxSizing:"border-box" };
    ...
    <ul style={{listStyle:"none",margin:0,padding:0}}>
      {bullets.map((b,i)=>(
        <li key={i} style={{display:"flex",alignItems:"flex-start",gap:6,marginBottom:3}}>
          <span style={{color:A,fontSize:10,marginTop:2,flexShrink:0,fontWeight:900}}>—</span>
          {editing?(
            <div style={{flex:1,display:"flex",gap:4}}>
              <input value={b} onChange={e=>{const n=[...bullets];n[i]=e.target.value;onChange(n);}} style={{flex:1,fontSize:11,color:CT,background:S3,border:`1px solid ${hex2rgba(A,0.3)}`,borderRadius:2,padding:"1px 4px",outline:"none",fontFamily:"inherit"}}/>
...
function TagList({ tags, onChange, editing }:{ tags:string[]; onChange:(t:string[])=>void; editing:boolean }) {
  ...
  return (
    <div style={{display:"flex",flexWrap:"wrap",gap:4}}>
      {tags.map((t,i)=>(
        <div key={i} style={{backgroundColor:S3,border:`1px solid ${hex2rgba(A,0.2)}`,color:CB,borderRadius:2,padding:"2px 8px",fontSize:10,display:"flex",alignItems:"center",gap:3}}>
          {t}
      ...
      {editing&&<input value={nv} onChange={e=>setNv(e.target.value)} onKeyDown={e=>{if(e.key==="Enter"&&nv.trim()){onChange([...tags,nv.trim()]);setNv("");}}} style={{width:70,fontSize:10,border:`1px solid ${hex2rgba(A,0.4)}`,borderRadius:2,padding:"2px 6px",background:hex2rgba(A,0.05),outline:"none",color:CT,fontFamily:"inherit"}} placeholder="+ Neu"/>}
    </div>
  );
}
  ...
  return (
    <div style={{marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
        <div style={{width:4,height:16,backgroundColor:A,flexShrink:0}}/>
        <span style={{fontSize:11,fontWeight:800,letterSpacing:"0.12em",textTransform:"uppercase" as const,color:"#111"}}>{title}</span>
...
export default function CV_Magazine() {
  const {data,setData,fontKey,setFontKey,sizeKey,setSizeKey,photoShapeKey,setPhotoShapeKey,photoSrc,setPhotoSrc,clrs,setClrs,resetStorage}=useCVStorage("magazine",DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e5e5e5",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8,flexWrap:"wrap"}}>
        <button onClick={()=>setEditing(e=>!e)} style={{display:"flex",alignItems:"center",gap:6,padding:"7px 16px",borderRadius:3,fontSize:13,fontWeight:700,cursor:"pointer",border:`2px solid ${editing?"#16a34a":A}`,backgroundColor:editing?"#16a34a":A,color:"white"}}>
          {editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}
        </button>
        <button onClick={()=>window.print()} style={{padding:"7px 16px",borderRadius:3,fontSize:13,fontWeight:700,cursor:"pointer",border:`2px solid #111`,backgroundColor:"#111",color:"white",display:"flex",alignItems:"center",gap:6}}>
          <PrinterIcon style={{width:16,height:16}}/>Drucken
        </button>
        <button onClick={()=>setShowDesign(v=>!v)} style={{padding:"7px 16px",borderRadius:3,fontSize:13,fontWeight:700,cursor:"pointer",border:`2px solid ${showDesign?A:"#ccc"}`,backgroundColor:showDesign?hex2rgba(A,0.08):"white",color:showDesign?A:"#555"}}>
          🎨 Design
        </button>
        <button onClick={()=>{resetStorage();setData(JSON.parse(JSON.stringify(DEFAULT_CV_DATA)));setPhotoSrc("");setFontKey("nunito");setSizeKey("md");setPhotoShapeKey("circle");setClrs(DEFAULT_COLORS);setShowDesign(false);}} style={{padding:"7px 16px",borderRadius:3,fontSize:13,fontWeight:700,cursor:"pointer",border:"2px solid #ccc",backgroundColor:"white",color:"#999",display:"flex",alignItems:"center",gap:6}}>
          <XMarkIcon style={{width:16,height:16}}/>Reset
        </button>
        {showDesign&&(
          <div style={{width:"100%",background:"white",border:"2px solid #eee",borderRadius:6,padding:"14px 18px",display:"flex",gap:24,flexWrap:"wrap"}}>
            <div><div style={{fontSize:10,fontWeight:800,color:"#999",marginBottom:7,letterSpacing:"0.12em",textTransform:"uppercase"}}>Schriftart</div><div style={{display:"flex",gap:5,flexWrap:"wrap"}}>{FONTS.map(f=><button key={f.key} onClick={()=>setFontKey(f.key)} style={{padding:"3px 9px",borderRadius:3,border:`2px solid ${fontKey===f.key?A:"#ddd"}`,background:fontKey===f.key?hex2rgba(A,0.08):"transparent",color:fontKey===f.key?A:"#888",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:f.family}}>{f.label}</button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:800,color:"#999",marginBottom:7,letterSpacing:"0.12em",textTransform:"uppercase"}}>Schriftgröße</div><div style={{display:"flex",gap:5}}>{FONT_SIZES.map(s=><button key={s.key} onClick={()=>setSizeKey(s.key)} style={{padding:"3px 12px",borderRadius:3,border:`2px solid ${sizeKey===s.key?A:"#ddd"}`,background:sizeKey===s.key?hex2rgba(A,0.08):"transparent",color:sizeKey===s.key?A:"#888",fontSize:11,fontWeight:700,cursor:"pointer"}}>{s.label}</button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:800,color:"#999",marginBottom:7,letterSpacing:"0.12em",textTransform:"uppercase"}}>Foto-Form</div><div style={{display:"flex",gap:6,flexWrap:"wrap"}}>{PHOTO_SHAPES.map(s=><button key={s.key} onClick={()=>setPhotoShapeKey(s.key)} title={s.label} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"5px 7px",borderRadius:4,border:`2px solid ${photoShapeKey===s.key?A:"#ddd"}`,background:photoShapeKey===s.key?hex2rgba(A,0.06):"transparent",cursor:"pointer"}}><div style={{width:20,height:s.key==="ellipse"?26:20,borderRadius:s.br,clipPath:s.clip??"",backgroundColor:photoShapeKey===s.key?A:"#ccc"}}/><span style={{fontSize:8,color:photoShapeKey===s.key?A:"#999",whiteSpace:"nowrap"}}>{s.label}</span></button>)}</div></div>
            <div><div style={{fontSize:10,fontWeight:800,color:"#999",marginBottom:7,letterSpacing:"0.12em",textTransform:"uppercase"}}>Farben</div><div style={{display:"flex",gap:10,flexWrap:"wrap",alignItems:"flex-end"}}>{([{k:"A" as const,l:"Akzent"},{k:"BG" as const,l:"Hintergrund"},{k:"S2" as const,l:"Sektion"},{k:"S3" as const,l:"Karte"},{k:"SBG" as const,l:"Sidebar"},{k:"CT" as const,l:"Titel"},{k:"CB" as const,l:"Text"},{k:"CM" as const,l:"Gedimmt"}]).map(({k,l})=>(<label key={k} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer"}}><input type="color" value={clrs[k]} onChange={e=>setClrs(c=>({...c,[k]:e.target.value}))} style={{width:32,height:32,padding:2,borderRadius:4,border:"2px solid #ddd",cursor:"pointer",background:"none"}}/><span style={{fontSize:9,color:"#999",fontWeight:700}}>{l}</span></label>))}<button onClick={()=>setClrs(DEFAULT_COLORS)} style={{padding:"4px 10px",borderRadius:3,fontSize:11,fontWeight:700,border:"2px solid #ddd",background:"transparent",color:"#aaa",cursor:"pointer",alignSelf:"center"}}>↺ Reset</button></div></div>
          </div>
        )}
      </div>
      ...
      <div className={`${PFX}-doc`} style={{width:850,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 30px rgba(0,0,0,0.12)",overflow:"hidden",fontFamily:fnt}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          ...
          <div style={{backgroundColor:"#111",padding:"14px 40px",display:"flex",alignItems:"center",justifyContent:"space-between"}}>
            <span style={{fontSize:9,color:"#888",letterSpacing:"0.2em",textTransform:"uppercase"}}>CURRICULUM VITAE</span>
            <div style={{width:40,height:3,backgroundColor:A}}/>
            <span style={{fontSize:9,color:"#888",letterSpacing:"0.2em",textTransform:"uppercase"}}>{new Date().getFullYear()}</span>
          </div>
          <div style={{height:4,backgroundColor:A}}/>
          {/* Header */}
          <div style={{background:S2,padding:"30px 40px 24px"}}>
            <div style={{display:"flex",gap:24,alignItems:"flex-start"}}>
              <div style={{width:curShape.w,height:curShape.h,borderRadius:curShape.br,clipPath:curShape.clip??"",overflow:"hidden",flexShrink:0,backgroundColor:S3,border:`3px solid ${A}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:editing?"pointer":"default"}} onClick={()=>editing&&photoInputRef.current?.click()}>
              ...
              <div style={{flex:1}}>
                {editing?<input value={data.personal.name} onChange={e=>setP({name:e.target.value})} style={{display:"block",fontSize:40,fontWeight:900,color:CT,lineHeight:1.05,marginBottom:3,fontFamily:fnt,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.35)}`,borderRadius:2,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:40,fontWeight:900,color:CT,lineHeight:1.05,marginBottom:3}}>{data.personal.name}</div>}
                <div style={{height:3,width:80,backgroundColor:A,marginBottom:8}}/>
                {editing?<input value={data.personal.subtitle} onChange={e=>setP({subtitle:e.target.value})} style={{display:"block",fontSize:13,color:A,fontWeight:700,marginBottom:10,letterSpacing:"0.08em",fontFamily:fnt,background:hex2rgba(A,0.06),border:`1px solid ${hex2rgba(A,0.35)}`,borderRadius:2,padding:"2px 6px",outline:"none",width:"100%",boxSizing:"border-box"}}/>
                  :<div style={{fontSize:13,color:A,fontWeight:700,marginBottom:10,letterSpacing:"0.08em",textTransform:"uppercase"}}>{data.personal.subtitle}</div>}
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,marginTop:8}}>
                  {(["email","phone","location","github","linkedin","website"] as const).map(k=>editing
                    ?<input key={k} value={data.personal[k]??""} onChange={e=>setP({[k]:e.target.value} as Partial<CVData["personal"]>)} style={{fontSize:10,color:CB,background:hex2rgba(A,0.05),border:`1px solid ${hex2rgba(A,0.25)}`,borderRadius:2,padding:"2px 8px",outline:"none",fontFamily:"inherit"}}/>
                    :data.personal[k]?<span key={k} style={{fontSize:10,color:CM,backgroundColor:S3,borderRadius:2,padding:"2px 8px",border:"1px solid #ddd"}}>{data.personal[k]}</span>:null
                  )}
                </div>
              </div>
            </div>
          ...
          <div style={{display:"flex"}}>
            <div style={{flex:1,padding:"24px 28px 40px 40px",minWidth:0}}>
              <div style={{marginBottom:22}}>
                <SecH title="Projekte"/>
                {data.projects.map(p=>(
                  <div key={p.id} style={{marginBottom:12,paddingBottom:10,borderBottom:`1px solid ${S3}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2,gap:8}}>
                      ...
                      <E value={p.period} onChange={v=>updProj(p.id,{period:v})} editing={editing} style={{fontSize:10,color:A,flexShrink:0,fontWeight:700}}/>
                    </div>
              ...
              <div style={{marginBottom:22}}>
                ...
                {data.experience.map(ex=>(
                  <div key={ex.id} style={{marginBottom:14,paddingBottom:12,borderBottom:`1px solid ${S3}`}}>
                    <div style={{display:"flex",justifyContent:"space-between",marginBottom:2,gap:8,alignItems:"flex-start"}}>
                      <div>
                        ...
                        <E value={ex.company} onChange={v=>updExp(ex.id,{company:v})} editing={editing} style={{fontSize:12,color:A,fontWeight:700}}/>
                      </div>
                    ...
                    <E value={ex.description??""} onChange={v=>updExp(ex.id,{description:v})} editing={editing} style={{fontSize:11,color:CM,display:"block",marginBottom:3,marginTop:3}} placeholder="Beschreibung..."/>
              ...
              <div>
                ...
                {data.education.map(e=>(
                  <div key={e.id} style={{marginBottom:12,paddingBottom:10,borderBottom:`1px solid ${S3}`}}>
                    ...
                    <E value={e.institution} onChange={v=>updEdu(e.id,{institution:v})} editing={editing} style={{fontSize:12,color:A,fontWeight:700,display:"block",marginBottom:2}}/>
