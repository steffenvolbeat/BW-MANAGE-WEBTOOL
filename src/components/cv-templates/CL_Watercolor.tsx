"use client";
// ─── Anschreiben Template: Watercolor (Soft Painted Washes) ─────────────────
...
const DEFAULT_COLORS = { A:"#7b9ea0", BG:"#fdfaf5", HD:"#f0ebe2", S2:"#e8e0d4", CT:"#2a1f18", CB:"#3a2e24", CM:"#8a7a6a" };
...
const PFX="clwclr";
const ROSE="#d4808a"; const SAGE="#7b9a80"; const GOLD="#b8963c"; const BLUE="#7090b0";
...
function E({value,onChange,editing,multiline=false,style={} as React.CSSProperties,placeholder="...",rows=4}:{value:string;onChange:(v:string)=>void;editing:boolean;multiline?:boolean;style?:React.CSSProperties;placeholder?:string;rows?:number;}){
  ...
  const s:React.CSSProperties={...style,background:hex2rgba(A,0.06),border:`1px dashed ${A}55`,padding:"2px 5px",outline:"none",width:"100%",fontFamily:"inherit",fontSize:"inherit",color:"inherit",lineHeight:"inherit",fontWeight:"inherit",boxSizing:"border-box"};
...
export default function CL_Watercolor(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clwclr_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey]=usePersistentCLState('cl_clwclr_font',"nunito");
  const [sizeKey]=usePersistentCLState('cl_clwclr_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clwclr_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#e8e0d4",padding:"24px 16px",fontFamily:fnt}}>
      <style>{`
      ...
      <div className={`${PFX}-ctrl`} style={{maxWidth:850,margin:"0 auto 16px",display:"flex",gap:8}}>
        <button onClick={()=>setEditing(e=>!e)} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${SAGE}`,background:editing?SAGE:"transparent",color:editing?"white":SAGE,display:"flex",alignItems:"center",gap:6}}>{editing?<CheckIcon style={{width:16,height:16}}/>:<PencilSquareIcon style={{width:16,height:16}}/>}{editing?"Fertig":"Bearbeiten"}</button>
        <button onClick={()=>window.print()} style={{padding:"7px 16px",fontSize:13,cursor:"pointer",border:`2px solid ${ROSE}`,background:"transparent",color:ROSE,display:"flex",alignItems:"center",gap:6}}><PrinterIcon style={{width:16,height:16}}/>Drucken</button>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(100,80,60,0.12)",overflow:"visible"}}>
        <div className={`${PFX}-zoom`} style={{width:Math.round(850/scale),zoom:scale}}>
          {/* Watercolor wash header */}
          <div style={{background:`linear-gradient(135deg, ${hex2rgba(ROSE,0.25)} 0%, ${hex2rgba(BLUE,0.15)} 40%, ${hex2rgba(SAGE,0.25)} 100%)`,padding:"36px 48px 28px",position:"relative"}}>
            <div style={{position:"absolute",top:0,left:0,right:0,bottom:0,background:`radial-gradient(ellipse at 30% 50%, ${hex2rgba(GOLD,0.15)} 0%, transparent 60%)`}}/>
            <div style={{position:"relative"}}>
              <E value={data.personal.name} onChange={v=>setP({name:v})} editing={editing} style={{fontSize:28,fontWeight:700,color:CT,display:"block",letterSpacing:"0.02em"}}/>
              <E value={data.personal.subtitle} onChange={v=>setP({subtitle:v})} editing={editing} style={{fontSize:11,color:ROSE,display:"block",marginTop:4,fontStyle:"italic"}}/>
          ...
          <div style={{height:3,background:`linear-gradient(90deg, ${ROSE}, ${GOLD}, ${SAGE}, ${BLUE})`}}/>
          ...
          <div style={{padding:"28px 48px 48px"}}>
            ...
            <div style={{marginBottom:18,paddingBottom:10,borderBottom:`1px solid ${hex2rgba(SAGE,0.4)}`}}>
              <E value={data.subject} onChange={v=>setData(d=>({...d,subject:v}))} editing={editing} style={{fontSize:13,fontWeight:700,color:SAGE}}/>
            </div>
            ...
            {editing&&<button type="button" onClick={()=>setData(d=>({...d,bodyParagraphs:[...d.bodyParagraphs,""]}))} style={{fontSize:11,color:SAGE,background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:4,marginBottom:14}}><PlusIcon style={{width:12,height:12}}/>Absatz</button>}
            <div style={{marginTop:20}}>
              ...
              <div style={{height:2,width:80,background:`linear-gradient(90deg, ${ROSE}, ${SAGE})`,marginBottom:8,opacity:0.7,borderRadius:2}}/>
