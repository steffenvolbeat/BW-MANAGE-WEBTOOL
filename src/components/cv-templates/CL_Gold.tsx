...
const DEFAULT_COLORS = { A:"#fbbf24", BG:"#0f0a00", HD:"#272012", S2:"#1c1400", CT:"#ffffff", CB:"#d1d5db", CM:"#9ca3af" };
...
export default function CL_Gold() {
  const [data, setData] = usePersistentCLState<CLData>('cl_gold_data', JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing, setEditing] = useState(false);
  const [fontKey, setFontKey] = usePersistentCLState('cl_gold_font', "nunito");
  const [sizeKey, setSizeKey] = usePersistentCLState('cl_gold_size', "md");
  const [showDesign, setShowDesign] = useState(false);
  const [clrs, setClrs] = usePersistentCLState('cl_gold_colors', DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{ minHeight: "100vh", background: "#0f0a00", padding: "24px 16px", fontFamily: fnt }}>
      ...
      <div className="clgol-doc" style={{ width: 850, margin: "0 auto", backgroundColor: BG, boxShadow: `0 0 60px ${hex2rgba(A,0.15)}`, overflow: "hidden" }}>
