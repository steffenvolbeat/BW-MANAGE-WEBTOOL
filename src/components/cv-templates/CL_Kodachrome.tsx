"use client";
// ─── Anschreiben Template: Kodachrome ─────────────────────────────────────────────
...
const DEFAULT_COLORS = { A:"#e53935", BG:"#fffdf8", HD:"#fff8f0", S2:"#fff0e8", CT:"#1a0a00", CB:"#2a1000", CM:"#a06050" };
...
export default function CL_Kodachrome(){
  const [data,setData]=usePersistentCLState<CLData>('cl_clkoda_data',JSON.parse(JSON.stringify(DEFAULT_CL_DATA)));
  const [editing,setEditing]=useState(false);
  const [fontKey,setFontKey]=usePersistentCLState('cl_clkoda_font',"nunito");
  const [sizeKey,setSizeKey]=usePersistentCLState('cl_clkoda_size',"md");
  const [clrs,setClrs]=usePersistentCLState('cl_clkoda_colors',DEFAULT_COLORS);
  ...
  <ColCtx.Provider value={clrs}>
    <div style={{minHeight:"100vh",background:"#f0e8d8",padding:"24px 16px",fontFamily:fnt}}>
      ...
      <div className={`${PFX}-doc`} style={{width:850,minHeight:1202,margin:"0 auto",backgroundColor:BG,boxShadow:"0 4px 20px rgba(229,57,53,0.2)",overflow:"visible",position:"relative"}}>
